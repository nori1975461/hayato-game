...
const SFX = {
  kill: (combo) => {
    ...
    noise(0.18, 0.14, 3800, 'bandpass', 0, 450); // ズシャアッ！と斬りとばす長めのスイープ
    beep(520 + Math.min(combo, 12) * 45, 0.09, 'triangle', 0.05, 950 + combo * 45);
    beep(200, 0.09, 'sawtooth', 0.03, 60); // うっすら低音（重さだけ残す）
  },
  // ズバッ！と切った感触の音。主役は高→低へスイープする風切りノイズ（イース風）
  slash: () => {
    noise(0.13, 0.14, 4800, 'bandpass', 0, 700); // ズバァッ（ノイズが高い音から低い音へ流れる）
    beep(2400, 0.09, 'sawtooth', 0.035, 500);    // 刃のシャッという鳴り（控えめ）
    beep(240, 0.07, 'triangle', 0.035, 70);      // かすかな手ごたえ（コツン音は廃止）
  },
