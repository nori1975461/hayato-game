...
1. **成長するのは腕だけ。** 少年の体・服・顔・背丈は3段階を通じて完全に同一。強くなるのは本人ではなく、
   ...
   → キャンバスは **16×18 → 20×18 → 24×18**（幅が広がるのは腕がせり出すぶんだけ・§24.5）。
   表示スケールは**全段 3.0 固定**（R12までの「段が上がるほど本体も大きくなる」は撤回）。彼は成長しない。
...
## 24.2 腕の被覆は「肩から下」（R15cでユーザーが前腕型から改定）

初版は前腕型（肘から先・ユーザー選択）だったが、R15c で**肩から下**へ拡大（ユーザー指示
「レベルアップする部分の領域を広げよう。肘から下ではなく、肩から下まで」）。正典との整合はむしろ
上がる＝守り手は**両腕ごと**千切れたのだから、腕一式（肩から先）が戻るのが本来の姿。
頭・胴・脚が生身のままなので「機械に飲まれた戦士」ではなく**「腕を借りている少年」**は崩れない。

| 段 | 腕 | 幅（3×表示） | 心臓の金(c) | スチール(s) |
|---|---|---|---|---|
| Stage1 | 手首の「かけら」＋小さな拳 | 16ドット（48px） | 1px | 2px |
| Stage2 | 前腕の半ばまで＋拳がせり出す | 20ドット（60px） | 6px | 10px |
| Stage3 | 守り手の腕の本来の形＝こぶしが頭より大きい | 24ドット（72px） | 13px | 22px |
...
- 数値確認：3段で **衣 w=52 が完全に同一**（＝少年の体は不変）。肌 f は 34→32→32 で、Stage2以降は
  **装甲が上腕を2ドット覆う**ぶんだけ減る（＝前腕型の成長そのもの。少年が痩せたのではない）。
  厳密な不変条件は §24.3 のガードが定義する ①頭 0-5行 ②脚と裾 13-17行 ③胴の芯（中央16列の5〜10列）

## 24.5 R15b：レベルアップが実感できない問題（腕を「横へ」伸ばして解決）

初版（3段とも16×18・全段同幅）への実プレイFB「**レベルアップした姿が、レベルアップを実感できない。
違いが認識できない**」。実測すると Stage1→2 の差分は全画素の **8.6%（10ドット）**、シルエット
（輪郭幅）は3段とも48pxで不変だった。人間が一瞬で認識するのは色の増減ではなく**輪郭の変化**なので、
これでは読めなくて当然（R12「3段の差が小さい」と同じ問題の再発。当時は本体ごと拡大で解いたが、
それは R15 の「彼は成長しない」で撤回した手）。

**検討した選択肢**：(A) 腕を横へ伸ばす（キャンバス幅 16→20→24・少年は中央16列に固定のまま）
(B) 本体ごと拡大＝R12方式（正典の核と矛盾・却下）(C) オーラ等の演出だけ強化（輪郭が変わらず主解に
ならない・却下）(D) 色の増量だけ（今回失敗した当の方法・却下）。

**採用＝A**。正典に完全に忠実なまま（変わるのは腕だけ・少年の画素は不変・スケール3.0固定のまま）
シルエットが段階で変わる。しかも物語上より正しい：**守り手の腕は大人の機械の腕**であり、12歳が
着ければ本人より腕のほうが大きいのが本来の姿。Stage3 のこぶしは少年の頭より大きい
（＝かつてポッドを抱きとめた腕をそのまま着けている）。

| 指標 | 初版 | R15b |
|---|---|---|
| 幅 | 48 / 48 / 48px | **48 / 60 / 72px** |
| Stage1→2 差分 | 10px（8.6%） | **33px（24.3%）** |
| Stage1→3 差分 | 24px（18.5%） | **61px（37.2%）** |
| 金 c | 1→3→7 | 1→**6**→**13** |
| スチール s | 2→5→11 | 2→**10**→**22** |
| 衣 w（不変の証明） | 52 / 52 / 52 | 52 / 52 / 52 |

- ガードは §24.3 を改訂：幅 `[16,20,24]`・高さ18・**少年の芯はキャンバス中央の16列**に置き
  （`off=(幅-16)/2`）、中央スライス後に旧ルールで一致判定。
- Run.js：構えの拳の押し出しベースを `16 + 7/段`（Stage1=16/2=23/3=30）。体半幅が 24/30/36px に
  広がったので、忘れると構えの拳が描いた腕の中に沈む。
- **教訓：段階差は「差分画素の割合」と「輪郭幅の変化」で事前に数値検証する。** 色だけの差分は
  実プレイの縮尺では存在しないのと同じ（差分8.6%は読めず、24%＋幅25%増で読めた）。

## 24.6 R15c：肩から下へ拡大＋「モダンでスタイリッシュ」の造形文法

R15b への実プレイFB「視認できるようになったが、**スタイリッシュさがない**。もっとモダンで
スタイリッシュに。変化部分を肘から下ではなく**肩から下まで**広げよう」。

**失敗2回から確定した造形文法（18px級スプライトの腕の描き方）**：

1. ❌ **試作1＝砂時計（肩当て張り→肘2pxに絞り→拳flare）**：肘が細すぎて腕が途切れ、肩当てと拳が
   「翼／背負い物」に分離して見えた。行間の接続は**4ドット以上**重ねないと塊が割れる。
2. ❌ **試作2＝逆三角形（肩を横に大きく張る）**：肩の水平スラブが体幅を大きく超えると、下に支える
   質量がないため**マント/翼**に見える。腕として読ませる質量は**縦**に積む。
3. ✅ **確定＝縦スラブ型**：①肩プレート2行（胴への橋渡し）②**脇の下に1pxの空隙**（これが「体から
   生えた瘤」と「垂れ下がる腕」を分ける決定打）③縦長の腕スラブ（S2=4×7・S3=6×8ドット）
   ④**肘の暗い継ぎ目**（`d #232c3d`・暗色は装甲の内側だけ＝外縁に使うと背景#0a0a1eに溶ける）
   ⑤外縁は常に明色 s のリムライト（シルエットを立てる）⑥左腕だけ金の膝ナックル＋熱の筋（心臓側）。

| 指標 | S1 | S2 | S3 |
|---|---|---|---|
| 実輪郭幅 | 36px | 48px | 66px |
| 腕の構造 | 手首のかけら | 肩プレート＋4幅スラブ | 肩が首横まで＋6幅スラブ |
| Stage1との差分 | ― | 約30% | 約44% |

- ガード改定：肩当てが首の横（5行目）まで立ち上がるため、不変範囲の「頭」を **0-4行** に精密化
  （5行目の中央は胴の芯としてチェック継続）。衣 w=52 は3段とも不変のまま。
- 拡大率3.0固定・当たり判定 radius7 は据え置き（見た目だけの変更）。

## 24.7 R16：ブレイブギア（インタビュー駆動の全面刷新・正典の灰縛りを解除）

R15c への実プレイFB「**うーん。いまいち。**」→ ユーザーの指示で**4問インタビュー**を実施し、
回答から再設計した（推測での試作を止め、要求を先に確定させた）。

**インタビューの回答（設計の根拠）**：
1. いまいちの正体＝**全部**（腕の形・色が地味・少年本体が地味・ポーズが棒立ち）→ 部分修正ではなく全面刷新
2. 腕の系統＝**王道ヒーローロボ系**（張り出す肩・角ばった大きな拳）
3. 色＝**完全に自由**（正典§22の「敵と同じ灰・違いは光だけ」の縛りを解除してよい）
4. 参考＝**ロックマンエグゼ系**

**設計の翻訳**：
- 配色＝コバルト `b#2f6fe4` × 白装甲 `w#f2f4f8` × 金 `c#ffd23f`（エグゼ直系）。
  画面上の他要素（敵=灰銅 / モビット=パステル小球 / ジェム=緑・紫 / 回復=桃）とどれとも被らない
  唯一の色域なので、派手さと識別性が同時に立つ。輝度：スーツ105 / 白244 vs 背景12.3。
- 少年本体＝青ヘッドバンド（S3で金クレスト）＋黒アンダースーツの襟とベルト `d#232c3d`＋
  胸に2×2の金コア（＝守り手の心臓）＋白い大型ブーツ。**髪・顔・目は生身のまま不変**。
- ポーズ＝**A字の踏み込みスタンス**（両脚を開き、ブーツを外へ）。本体に拳は描かない
  （拳は playerFistImg が常時狙い角へ構える＝役割分担・二重表示を避ける）。
- 左右非対称の正典は**色で維持**：心臓側（画面左）の腕だけ金シーム＋アンバー熱、
  反対側は冷たいスカイ `s#9fe0ff` のエッジ。
- 拳（HERO_FISTS）＝白いナックルブロック＋**平らな金の打撃面**（+X側）。白はヒートtint
  （0xffd23f乗算）で素直に金へ振れる＝旧・青灰ベースより濁らない。
- オーラも段階色：S1コバルト 0x4f8cff → S2スカイ 0x9fe0ff → S3金 0xffd23f（Run.js）。

**このラウンドで再確認した造形文法（§24.6に追加）**：
- ❌ 白パネル＋大きい金角を胸の広い面積に置くと「よだれかけ」になる → 胸の記号は**2×2の小さな
  エンブレム**＋暗色の襟で締める。
- ❌ 腕を1〜2pxの斜め線で描くと質量が出ず、肩当てが再び翼化する → **腕は3〜5px幅の縦柱**で
  胴に直結し、分離は空隙ではなく**内側の暗シーム d の縦線**で描く（輪郭は連続させる）。
- ❌ 肩当てを1行の横板にすると翼になる（§24.6-2の再発）→ パウルドロンは**2行以上の高さのブロック**
  にして、直下の腕の柱と幅を揃える。
- ❌ 拳を両端テーパーで描くと「熱帯魚」になる → 角の1px落としだけの**ほぼ長方形**＋全高の
  平らな打撃面。
- 金シームは輪郭の外縁に置くと「金の物体を持っている」ように見える → **外縁の1px内側**に格納。

| 指標 | S1 | S2 | S3 |
|---|---|---|---|
| キャンバス | 16×18 | 20×18 | 24×18 |
| 総ドット | 108 | 148 | 174 |
| 金 c | 5 | 10 | 16 |
| 白 w | 18 | 28 | 38 |
| 平均輝度 | 134.8 | 141.0 | 147.3 |
| 段の記号 | バンド＋胸コア＋踏み込み | 白パウルドロン＋装甲腕＋白ポッド | 2行パウルドロン＋金クレスト＋金ポッド＋手首まで装甲 |

- ガード改定（validate-data）：2行目＝ヘッドバンドは**ギア**なので不変チェックから除外
  （S3の金クレスト昇格を許可）。3行目は芯5〜10列のみ（耳ポッド S2白→S3金 は芯の外）。
  髪(0-1)・顎(4)・脚とスタンス(13-17)は中央16列で完全一致、胴の芯(5-12)は襟・コア・ベルト込みで一致。
- 「白い衣」→「ブレイブスーツ」への変更は見た目優先のユーザー決定。物語上の説明（ゆりかごが
  編んだ強化スーツ等）は正典§22の追補として今後整える。
