...
async function main() {
  ...
  spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-first-run', `--remote-debugging-port=${DBG}`,
    '--window-size=700,420', `--user-data-dir=${path.join(HERE, '.chrome-prof-r29f')}`, 'about:blank'], { stdio: 'ignore' });
  let wsUrl = null;
  ...
  // ★通しの検証：マオウレクスを撃破 → エンディング → リザルト、と実際に遷移するか
  await evalJs(`(function(){ window.VORTEX.autotest=false; return 1; })()`);
  await evalJs(`(function(){var b=window.__run.boss; b.onBossKilled(b.entity); return 1;})()`);
  let scenes = '';
  for (let i = 0; i < 80; i++) {
    scenes = await evalJs(`window.__vortexGame.scene.getScenes(true).map(function(s){return s.scene.key;}).join(',')`);
    if (scenes === 'Ending') break;
    await sleep(100);
  }
  console.log('after boss kill ->', scenes);
  await sleep(1500);
  const r2 = await send('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync(path.join(HERE, 'r29-end-fromrun.png'), Buffer.from(r2.data, 'base64'));
  await evalJs(`(function(){ var e=window.__vortexGame.scene.getScene('Ending'); e.toResult(); return 1; })()`);
  await sleep(800);
  console.log('after skip ->', await evalJs(`window.__vortexGame.scene.getScenes(true).map(function(s){return s.scene.key;}).join(',')`));
