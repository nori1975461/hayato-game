...
async function main() {
  ...
  spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
    ...
    `--user-data-dir=${path.join(HERE, '.chrome-prof-r42')}`, 'about:blank'], { stdio: 'ignore' });
  ...
  await evalJs(`(function(){
    ...
    window.__W = { back: 0, shot: 0, last: null };
  ...
  const fin = await evalJs(`(function(){
    ...
    return { wireShot状態: W.shot, wireBack状態: W.back,
