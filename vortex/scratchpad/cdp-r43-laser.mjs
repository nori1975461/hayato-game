...
async function main() {
  ...
  spawn(CHROME, ['--headless=new', '--disable-gpu', '--no-first-run', '--no-default-browser-check',
    ...
    `--user-data-dir=${path.join(HERE, '.chrome-prof-r43l')}`, 'about:blank'], { stdio: 'ignore' });
  ...
  await evalJs(`(function(){
    ...
    window.__watch = setInterval(function(){
      ...
      if (st !== D.last) {
        ...
        // ★R43 レーザーの発射時に「主人公が確定した射線からどれだけ離れていたか」を測る。
        //   旧実装は薙ぎの中心を発射の瞬間の主人公方向に取っていたので、ここは構造上ずっと0だった。
        if (st === 'laserFire' || st === 'chestFire' || st === 'alignFire' || st === 'alignFire2') {
          var bm = r.boss.debugBeam && r.boss.debugBeam();
          if (bm) {
            var pa = Math.atan2(r.player.y - b.y, r.player.x - b.x);
            var dd = Math.hypot(r.player.x - b.x, r.player.y - b.y);
            var off = Math.abs(((pa - bm.from + Math.PI * 3) % (Math.PI * 2)) - Math.PI);
            D.fires = D.fires || [];
            D.fires.push({ st: st, off: Math.round(Math.abs(Math.sin(off)) * dd),
              hw: Math.round(bm.width / 2 + r.player.radius) });
          }
        }
        D.last = st;
      }
      if (r.boss.locked) D.lockSeen = (D.lockSeen || 0) + 1;
  ...
  await evalJs(`(function(){
    ...
    window.__bot = setInterval(function(){
      ...
      // ★R43 「予告を見て避ける」プレイの再現：照準がロックされたら、射線と直角に全力で走る。
      //   ボットは本来これをしないので、旧実装と同じく必ず被弾する（避ける遊びの有無を測れない）。
      //   ⚠️ このボットは120ms間隔で回るので、1回で 148px/s × 0.12s = 17.8px 動かさないと
      //      実際の全力移動にならない（最初2.47px＝1フレーム分にしていて14%の速さだった）。
      if (r.boss.locked) {
        var side = (window.__D.dodgeDir || 1);
        r.player.x += (-dy / d) * 17.8 * side;
        r.player.y += (dx / d) * 17.8 * side;
        window.__D.dodged = (window.__D.dodged || 0) + 1;
        return;
      }
  ...
  if (!D) { console.log('計測に失敗（__D が取れない）'); }
    ...
    // ★R43 レーザーが「避けられる形」になったかの実測
    console.log('');
    console.log('--- R43 照準ロック ---');
    console.log('ロックを観測したフレーム数: ' + (D.lockSeen || 0)
      + ' / 回避に使ったフレーム: ' + (D.dodged || 0));
    var F = D.fires || [], esc = 0, byk = {};
    ...
    for (var k3 in byk) {
      var arr = byk[k3].slice().sort(function(x, y){ return x - y; });
      console.log('  ' + k3.padEnd(11) + ' ' + arr.length + '発  射線からの距離 中央値'
        + arr[Math.floor(arr.length / 2)] + 'px  最大' + arr[arr.length - 1] + 'px');
    }
    console.log('発射の瞬間に判定の外にいた: ' + esc + '/' + F.length + '発（旧実装は構造上0）');
    console.log('');
