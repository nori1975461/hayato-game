...
// 既定 GAIKA2＝候補 63（ふだん＝左肩は跳ね上げ・金）／GAIKA2_KEYDOWN＝候補 87（鍵が入った姿）／GAIKA2_FINAL＝候補 120／GAIKA2_FIRING＝候補 125（撃つ瞬間＝太いレーザー）（最終形態＝09-22 に跡＝窪み＋金具・三本目はマゼンタの光刃で確定）
import { createHash } from 'node:crypto';
import { GAIKA2, GAIKA2_KEYDOWN, GAIKA2_FINAL, GAIKA2_FIRING, gaika2With } from './gaika-candidates.mjs';
...
let ng = 0;
for (const [name, built, no] of [['GAIKA2', GAIKA2, 63], ['GAIKA2_KEYDOWN', GAIKA2_KEYDOWN, 87], ['GAIKA2_FINAL', GAIKA2_FINAL, 120], ['GAIKA2_FIRING', GAIKA2_FIRING, 125]]) {
  const c = CANDS[no];
