// 死守する制約の検査：既定（Z2＝ザク型のひねり）と比較用の四版とも世界 y 22（扇の付け根の平らな上端・|x|<=36）を胴の外周が覆っているか。node check-gaika2-fanroot.mjs
//   ドム版は中央に Λ の切れ込み（意図した開口）があるので、穴は数えず外周だけを見る
import * as M from './gaika-candidates.mjs';
let ok = true;
for (const [label, d] of [['Z2', M.GAIKA2], ['NZ', M.GAIKA2_NZ], ['DOM', M.GAIKA2_DOM], ['ZAKU', M.GAIKA2_ZAKU], ['ZEONG', M.GAIKA2_ZEONG]]) {
