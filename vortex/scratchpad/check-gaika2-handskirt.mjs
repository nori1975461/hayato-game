...
//   目安＝開いた手（画面左）25% 以下・垂らした手（画面右＝第48稿で添付の素描どおり肘からほぼ真下へ下げた＝スカートの前に下がるのが仕様）40% 以下・
//   胴の陰に隠れる画素が 0 で HANDSKIRT_OK（胴は腕より手前に描かれるので、隠れると指が欠けて見える）
...
for (const side of [-1, 1]) {
  ...
  const pct = Math.round((over / Math.max(1, tot)) * 100); if (pct > (side < 0 ? 25 : 40) || hid > 0) ok = false;
  out.push((side < 0 ? '画面左（開いた手）' : '画面右（垂らした手）') + ' かかる ' + over + 'px＝' + pct + '%' + (hid ? ' / 胴の陰 ' + hid + 'px' : ''));
}
console.log(ok ? 'HANDSKIRT_OK' : 'HANDSKIRT_NG', out.join(' ／ '), '（目安＝画面左 25%・画面右 40% 以下・胴の陰 0）');
