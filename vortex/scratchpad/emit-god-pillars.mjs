...
//   reveal＝最後の一瞬だけ本当の色で見せる「身体の4分の1」（左上を 0,0 とした割合・面積はちょうど 1/4）。
// ★2026-09-23 ユーザー指示「**上半身 特に顔ではなく 下半身をみせて。顔は黒のシルエットで**」＝
//   窓は**体の下半分**へ。顔にあたる部位の縦位置は `probe-god-parts-y.mjs` で測った（当て推量で置かない）：
//     大聖堂＝薔薇窓 0.17〜0.57（単眼）／玉座＝王冠 0.20〜0.33・王 0.35〜0.72（顔は上 1/3）／
//     軌道神核＝単眼 0.43〜0.79／骸華 第一案＝宝冠 0.28〜0.41・鈴 0.41〜0.48。
//   → 三柱は 0.6〜1.0 の帯（幅 0.625・中央）＝顔より下だけ。軌道神核だけは単眼が 0.79 まで下がるので
//     **下端の帯**（全幅 0.75〜1.0）にして眼を黒のまま残す。
const LIST = [
  { id: 'cathedral', name: '堕天の大聖堂', def: CATHEDRAL, rig: CATHEDRAL.rig,
    reveal: [0.1875, 0.6, 0.625, 0.4], revealNote: '身廊の下と腕と配線（薔薇窓＝顔は黒のまま）' },
  { id: 'throne', name: '腐蝕の玉座', def: THRONE, rig: THRONE.rig,
    reveal: [0.1875, 0.6, 0.625, 0.4], revealNote: '座と須弥壇（王冠と王の顔は黒のまま）' },
  { id: 'godcore', name: '軌道神核', def: { ...MAOU, sprites: MAOU.trueSprites }, rig: MAOU.trueRig,
    reveal: [0.0, 0.75, 1.0, 0.25], revealNote: '下の環（単眼＝顔は黒のまま）' },
  ...
  { id: 'gaika', name: '蒼神骸華', def: GAIKA, rig: GAIKA.rig,
    reveal: [0.1875, 0.6, 0.625, 0.4], revealNote: '蓮華座と脚と輪（宝冠と顔は黒のまま）' },
];
