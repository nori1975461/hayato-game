...
const ARML_W = 46, ARML_H = 113;
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (41,58)＝世界 (-17,-9)。肘 (29,72) → 拳 (18,64) が錫杖の柄を握る。錫杖は左へ傾いて立ち、頭（尖った金の輪の中に金の宝珠・深紅の遊環6つ）は冠より高い
  // 第16案：輪の先に蒼硝子の穂（刃・深紅の縁・月牙と同じ工房）を継ぐ＝獣を追い払う音の杖（殺さないための道具）に刃を付けた＝戒を捨てた悪神の証。梵鐘を金棒にしたのと同じ倒錯。
  //   顔は載せない（第15案の頭蓋は撤回・輪の中は第14案までの金の宝珠＝滑らかな金の卵）。穂のぶん上に T=13 伸ばす（以下の y は旧座標 + T）
  const T = 13;
  const G = g(ARML_W, ARML_H);
  const ax = (y) => 14 - (44 + T - y) * 0.11;                                                                              // 柄の軸（左へ傾く）
  ...
  for (let y = T - 1; y <= T + 2.5; y += 0.25) { const c = ax(y); for (let x = -1.8; x <= 1.8; x += 0.25) P(G, c + x, y, x < 0 ? 'G' : 'Y'); }   // 口金（輪の頂と穂の継ぎ目）
  for (let y = 24 + T; y <= 27 + T; y++) { const c = ax(y); for (let x = c - 2.6; x <= c + 2.6; x += 0.5) P(G, x, y, x < c ? 'G' : 'Y'); }   // 輪の座
  { // 穂：口金 (ax(T), T-0.5) から左上へ 12 伸びる木の葉形の刃（長さ：幅＝2.2）。身は蒼硝子（光側 P・影側 Q＝暗い背景に沈まない明るさ）・縁は深紅 0.9（血の刃）・切っ先に光。半幅は最大 2.7
    //   落とした稿＝長さ 9.7・縁 1.1・身 Q/q → 寸詰まりで縁が太く、等倍で「赤い葉」に見えた
    const bx = ax(T), by = T - 0.5, tx = bx - 4.0, ty = 0.5, BL = by - ty;
    for (let u = 0; u <= 1; u += 0.004) {
      const cx = bx + (tx - bx) * u, cy = by - BL * u, w = 2.7 * Math.pow(Math.sin(Math.PI * (0.12 + 0.88 * u)), 0.55);
      for (let k = -w; k <= w; k += 0.25) P(G, cx + k, cy, u > 0.94 ? 'A' : Math.abs(k) > w - 0.9 ? 'R' : k < -0.15 * w ? 'P' : 'Q');
    }
  }
  ...
  arm(G, [[41, 48 + T], [29, 62 + T], [18, 54 + T]], 4.5, 3.4);                                                            // 第13案：腕を 1/4 太く（3.6/2.7 → 4.5/3.4・胴 46 の 1/5）
  armlet(G, [41, 48 + T], [29, 62 + T], 0.5, 4.6, true); armlet(G, [29, 62 + T], [18, 54 + T], 0.72, 3.9);                  // 臂釧（深紅の玉）と腕釧＝金
  fist(G, 18, 53 + T, 4.0, 3.6);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝錫杖（獣を追い払う音の杖）に刃を継ぎ、梵鐘（鎮めの鐘）を金棒にした。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は尖った金の輪の先に蒼硝子の穂（深紅の縁）を継いだ錫杖＝殺さないための音の杖に刃を付けた戒破りの武具（穂の先は月牙と同じ高さ）。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は禍々しい月牙（深紅の縁・切っ先の光・後ろに影の月を従える）。後の左手は無く、右肩の上だけに翼の列が立つ＝左右非対称。冠の簾と帯の玉は紫＝最高位の色。'
...
function build(opts = {}) {   // opts.sword＝曲刀の中の左手（baseR）を付ける変種（第14案改の比較用・既定は無し）
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [41 / ARML_W, 61 / ARML_H] },
