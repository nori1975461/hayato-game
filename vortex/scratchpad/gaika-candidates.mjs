...
const GATLING = (() => {
  ...
  for (const s of [-1, 1]) {
    const q = [[-7, -6], [15, -12], [9, 4], [-7, 2]].map(([o, y]) => [X(s * (20 + o)), Y(-17 + y)]);   // [外向きの距離, y]：内上・切っ先・外下・内下
    ...
    for (let y = Y(-31); y <= Y(-11); y += 0.25) for (let x = X(s * 10) ; s * (x - X(s * 37)) <= 0; x += s * 0.25) {
      ...
      P(G, x, y, s * cr(q[0], q[2], x, y) < 0 ? 'R' : 'r');
    }
    LN(G, q[0][0], q[0][1], q[1][0], q[1][1], 'A'); LN(G, q[3][0], q[3][1], q[2][0], q[2][1], 'Y');
  }
  OUTLINE(G);
  return R(G);
})();
