...
const TORSO4 = (() => {
  ...
  for (let y = -40; y <= 38; y += 0.25) {
    const w = y < -33 ? 6 : y < -29 ? 14 + (y + 33) * 3.4 : y < -11 ? 27.6 : y < -3 ? 27.6 - (y + 11) * 0.95 : y < 18 ? 19 - (y + 3) * 0.3 : y < 22 ? 15.5 : 15 + (y - 22) * 0.45;   // 2回目：1回目の丸い胸は卵に見えた＝肩の張った角ばった胸・面取り
    for (let x = -w; x <= w; x += 0.25) {
      const lx = (x + w) / (2 * w);
      let c;
      if (y < -33) c = lx < 0.3 ? 'm' : 'j';
      else if (y < -3) c = y > -4.6 ? 'R' : Math.abs(Math.abs(x) - 16) < 0.5 ? 'k' : Math.abs(x) > 17.5 && Math.abs(x) < 24.5 && [-25, -22, -19].some((vy) => Math.abs(y - vy) < 0.55) ? 'k' : lx < 0.05 ? 'm' : lx < 0.44 ? 'j' : 'k';                   // 紺の胸（分割線・吸気の溝・下の縁に金）
      else if (y < 18) c = Math.abs(((y + 3) % 5.2) - 0.3) < 0.5 ? 'k' : lx < 0.12 ? 'm' : lx < 0.6 ? 'j' : 'k';   // 節のある黒い腹
      else if (y < 22) c = y < 19 ? 'Y' : 'y';   // くすんだ金の帯
      else c = y > 36.4 ? 'k' : lx < 0.05 ? 'm' : lx < 0.42 ? 'j' : 'k';   // 腰（第39稿：下縁の深紅の一本線を撤去＝スカートの前面を空ける）
      P(G, X(x), Y(y), c);
    }
  }
