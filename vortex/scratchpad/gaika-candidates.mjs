...
// =====================================================================
const ARMR_W = 38, ARMR_H = 60;
const ARM_R = (() => {
  // 骸華の左手（画面右）。肩 (3,4)＝世界 (17,-9)。腕をほぼ伸ばして下へ外へ（肘 (10,16)・拳 (15,26)＝世界 (29,13)＝腰の外）、金棒は拳から外へ 1:2 の傾きで斜め下へ（第12案改）。
  // 参考＝νガンダムの右手：腕を伸ばしてライフルを斜め下へ。柄の上端は前腕の後ろに隠す（見えるのは拳から下だけ）。
  // 鐘＝世界 x 33〜48・y +25〜+43＝座の右前（仰蓮の右端の前）。背景が密なので鐘は「太い黒の縁・暗い胴・金の口・深紅の帯一本・深紅の舌」だけの太い形にする（棘と袈裟襷は溶けるので落とす）
  const G = g(ARMR_W, ARMR_H), FX = 15, FY = 24, dx = 1 / Math.sqrt(5), dy = 2 / Math.sqrt(5), nx = dy, ny = -dx;         // 1:2 の傾き（ドットの段が揃う）。拳＝世界 (29,11)
  const Q = (t, s, ch) => P(G, FX + t * dx + s * nx, FY + t * dy + s * ny, ch);                                             // t＝軸に沿って下へ・s＝軸をまたいで外へ
  const W = (t, s) => [FX + t * dx + s * nx, FY + t * dy + s * ny];
  for (let t = -3; t <= 15; t += 0.25) for (let s = -1.5; s <= 1.5; s += 0.25) Q(t, s, s < -0.6 ? 'f' : s < 0.6 ? 'm' : 'j');   // 柄（拳の下だけ見える）
  for (let t = 3; t <= 13; t += 0.25) Q(t, 1, 'c');                                                                       // 柄の導管（神経光・連続線）
  for (let t = 8; t <= 9.5; t += 0.25) for (let s = -2; s <= 2; s += 0.25) Q(t, s, t < 8.75 ? 'G' : 'Y');                   // 金の環
  const B = 14;                                                                                                            // 竜頭の位置（軸上）
  const bw = (u) => u < 1.5 ? 1.3 : u < 5 ? 2.4 + 2.2 * ((u - 1.5) / 3.5) : u < 12.5 ? 4.6 + 0.3 * ((u - 5) / 7.5) : 4.9 + 1.4 * ((u - 12.5) / 4);   // 梵鐘の輪郭（竜頭・丸い肩・胴・口へ広がる）
  for (let u = -1; u <= 17.5; u += 0.25) { const w = bw(Math.min(16.5, Math.max(0, u))) + 1.0; for (let s = -w; s <= w; s += 0.25) Q(B + u, s, 'k'); }   // 鐘の下地＝太い黒の縁（密な背景から切り離す）
  for (let u = 0; u <= 16.5; u += 0.25) { const w = bw(u); for (let s = -w; s <= w; s += 0.25) { const v = s / w; Q(B + u, s, u < 1.5 ? 'G' : u >= 14 ? (v < -0.2 ? 'G' : 'Y') : v < -0.35 ? 'm' : v < 0.45 ? 'j' : 'k'); } }   // 胴は暗く単純に
  for (let s = -4.7; s <= 4.7; s += 0.25) for (let k = -1.0; k <= 1.0; k += 0.25) Q(B + 8.5 + k, s, k < 0 ? 'R' : 'r');                       // 胴の帯＝深紅一本
  for (let s = -4.8; s <= 4.8; s += 0.25) Q(B + 12.9, s, 'k');                                                            // 帯の溝
  for (let u = -1.4; u <= 1.4; u += 0.25) for (let s = -5.8; s <= 5.8; s += 0.25) if ((s / 5.8) ** 2 + (u / 1.4) ** 2 <= 1) Q(B + 17.4 + u, s, 'k');   // 口の中（斜め下から覗く空洞）
  { const [cx, cy] = W(B + 17.6, 0); DISC(G, cx, cy, 1.3, 'k'); DISC(G, cx, cy, 1.0, 'R'); P(G, cx - 0.4, cy - 0.4, 'A'); }   // 舌（深紅）
  arm(G, [[3, 4], [10, 15], [15, 24]], 3.6, 2.7);                                                                          // 腕は柄の上に描く＝柄の上端を隠す
  fist(G, 15, 23.5, 3.8, 3.4);                                                                                              // 拳は柄の上＝握り
  OUTLINE(G);
  return R(G);
})();
const ARML_W = 44, ARML_H = 100;
const ARM_L = (() => {
  ...
  arm(G, [[41, 48], [29, 62], [18, 54]], 3.6, 2.7);
  fist(G, 18, 53, 3.8, 3.4);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の死面を掲げた錫杖（月牙と同じ高さ）。左手（画面右）は腕を伸ばして金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための梵鐘を引き抜いたもので、座の右前に斜めに吊られる（暗い胴・金の口・深紅の帯・口の中に深紅の舌）。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。後の右手は大きな月牙、後の左手は芯が深紅の日輪。冠の簾と帯の玉は紫＝最高位の色。'
...
function build() {
  ...
  const rig = [
    ...
    { role: 'armR', tex: 'armR', ox: 17, oy: -9, origin: [3 / ARMR_W, 4 / ARMR_H] },
