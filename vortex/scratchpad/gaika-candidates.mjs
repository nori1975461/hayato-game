...
const HALO = (() => {
  ...
  for (const a of [226, 248, 270, 292, 314]) {                                                        // 宝珠（歯と歯の間・頂に5つ・頂の一つは深紅）
    ...
    DISC(G, sx, sy, 1.4, 'R'); P(G, sx - 0.5, sy - 0.5, 'A'); P(G, sx + 0.7, sy + 0.7, 'r');                            // 頂の宝珠5つは全部深紅
  }
  OUTLINE(G);
  return R(G);
})();
...
const TORSO = (() => {
  ...
  for (let y = 1; y < TORSO_H; y++) {
    ...
    for (let x = Math.ceil(cx - hw); x <= Math.floor(cx + hw); x++) {
      ...
      let ch = u < -0.55 ? 'm' : 'j';
      if (y === 13 || y === 21) ch = 'k';
      if (y === 14 || y === 22) ch = u < 0 ? 'R' : 'r';                                                       // 板の継ぎ目は赤糸縅（深紅の糸で板を綴る）
  ...
  for (const [dir, lit] of [[-1, true], [1, false]]) {                                                       // 肩当て（2層）＋牙
    for (let layer = 0; layer < 2; layer++) {
      ...
      for (let y = y0; y < y0 + 3; y++) for (let x = Math.min(x0, x1); x <= Math.max(x0, x1); x++) {
        let ch = y === y0 ? 'k' : y === y0 + 1 ? (lit ? 's' : 'f') : (lit ? 'R' : 'r');                       // 肩当ての下端も赤糸縅
...
const CROWN = (() => {
  ...
  P(G, 16, B + 1, 'k'); P(G, 23, B + 1, 'k'); P(G, 19, B + 1, 'R'); P(G, 20, B + 1, 'A');                            // 帯の中央の玉は深紅（一滴）・帯の青は黒に
  ...
  for (const x of [11, 13, 26, 28]) for (let y = B + 3; y <= B + 7; y++) P(G, x, y, y % 2 ? 'j' : 'k');                   // 玉の簾は黒鉄（冠の下は全部黒＝どの絵でも顔の位置に水色を残さない）
  OUTLINE(G);
  return R(G);
})();
...
// =====================================================================
const ARMR_W = 34, ARMR_H = 84;
const ARM_R = (() => {
  // 骸華の左手（画面右）。肩 (3,40)＝世界 (17,-9)。肘 (15,54) → 拳 (24,46) が柄を握る。垂直に持つ。
  // ひねり＝頭が梵鐘：祀って鎮めるための鐘を引き抜いて棍棒にした（口を下に・乳の鋲・口の縁に骨の棘＝金棒の名残・舌は深紅）。鐘＝世界 -49〜-33
  const G = g(ARMR_W, ARMR_H), sx = 24;
  for (let y = 19; y <= 79; y++) for (let x = sx - 1.5; x <= sx + 1.5; x += 0.5) P(G, x, y, x < sx - 0.6 ? 'f' : x < sx + 0.6 ? 'm' : 'j');   // 柄（垂直）
  for (let y = 21; y <= 76; y += 0.5) P(G, sx + 1, y, 'c');                                                                // 柄の導管（神経光・連続線）
  for (const y of [32, 50, 68]) for (let x = sx - 2; x <= sx + 2; x += 0.5) { P(G, x, y, 'G'); P(G, x, y + 1, 'Y'); }        // 金の環
  for (let y = 80; y <= 83; y++) for (let x = sx - 2.5; x <= sx + 2.5; x += 0.5) P(G, x, y, y === 80 ? 'G' : 'Y');           // 石突
  const bw = (y) => y < 1.5 ? 1.3 : y < 5 ? 2.4 + 2.2 * ((y - 1.5) / 3.5) : y < 12.5 ? 4.6 + 0.3 * ((y - 5) / 7.5) : 4.9 + 1.4 * ((y - 12.5) / 4);   // 梵鐘の輪郭（竜頭・丸い肩・胴・口へ広がる）
  for (let y = 0; y <= 16.5; y += 0.25) { const w = bw(y); for (let x = sx - w; x <= sx + w; x += 0.25) { const v = (x - sx) / w; P(G, x, y, y < 1.5 ? 'G' : y >= 14.5 ? (v < -0.2 ? 'G' : 'Y') : v < -0.45 ? 'f' : v < 0.15 ? 'm' : v < 0.65 ? 'j' : 'k'); } }
  for (const s of [-1, 1]) for (const y of [5.5, 8.5, 11.5]) bone(G, sx + s * 4.4, y, sx + s * 7.4, y - 1.4, 1.1, 0.4);      // 胴の両脇の骨の棘 3本ずつ（金棒の名残＝輪郭に出す）
  for (const dx of [-2.6, 2.6]) for (let y = 4.5; y <= 12.5; y += 0.25) for (let k = -0.75; k <= 0.75; k += 0.25) P(G, sx + dx + k, y, k < 0 ? 'R' : 'r');   // 袈裟襷（縦）＝深紅
  for (let x = sx - 4.6; x <= sx + 4.6; x += 0.25) for (let k = -0.75; k <= 0.75; k += 0.25) P(G, x, 8.5 + k, k < 0 ? 'R' : 'r');                  // 袈裟襷（横）＝深紅
  for (let x = sx - 4.8; x <= sx + 4.8; x += 0.25) P(G, x, 12.9, 'k');                                                     // 帯の溝
  ELL(G, sx, 17.4, 5.8, 1.4, 'k');                                                                                        // 口の中（下から覗く空洞）
  DISC(G, sx, 17.6, 1.3, 'k'); DISC(G, sx, 17.6, 1.0, 'R'); P(G, sx - 0.4, 17.2, 'A');                                     // 舌（深紅）
  arm(G, [[3, 40], [15, 54], [24, 46]], 3.6, 2.7);
  fist(G, 24, 45, 3.8, 3.4);
  OUTLINE(G);
  return R(G);
})();
const ARML_W = 44, ARML_H = 92;
const ARM_L = (() => {
  ...
  for (let y = 26; y <= 85; y++) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) P(G, x, y, x < c - 1.2 ? 'f' : x < c - 0.2 ? 'm' : x < c + 1.2 ? 'j' : 'k'); }   // 柄
  for (const y of [50, 66, 80]) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) { P(G, x, y, 'G'); P(G, x, y + 1, 'Y'); } }   // 金の環
  for (let y = 86; y <= 91; y++) { const c = ax(y); for (let x = c - 2.5; x <= c + 2.5; x += 0.5) P(G, x, y, y === 86 ? 'G' : 'Y'); }   // 石突
  ...
  arm(G, [[41, 40], [29, 54], [18, 46]], 3.6, 2.7);
  fist(G, 18, 45, 3.8, 3.4);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の死面を掲げた錫杖。左手（画面右）は垂直に立てた金棒＝頭は祀って鎮めるための梵鐘を引き抜いたもの（深紅の袈裟襷・口の中に深紅の舌・胴の脇に骨の棘）。後の右手は大きな月牙、後の左手は芯が深紅の日輪。'
  + '深紅は封印の内側から滲む神の色＝赤糸縅の鎧・紙垂・円光の宝珠・錫杖の遊環・環の錠・月牙の玉・死面の涙・胎の心臓・盃の流れ。';
...
const SEAL = (() => {
  ...
  const shide = (kx, y0) => { for (let y = 0; y < 6; y += 0.25) { const off = y < 2 ? 0 : y < 4 ? 1.3 : 0; for (let k = 0; k < 2.2; k += 0.25) P(G, kx + off + k - 1.1, y0 + y, y < 0.5 ? 'k' : k > 1.7 ? 'r' : 'R'); } };   // 紙垂＝稲妻形・深紅（封印の内側から神の色が滲む）・腹に目が行きすぎない大きさ
...
function build() {
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [41 / ARML_W, 40 / ARML_H] },
