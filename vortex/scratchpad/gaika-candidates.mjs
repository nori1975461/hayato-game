...
// 第21案：ユーザー指示「添付画像の鉄棒を再現して。ただし、柄を紅くして、鉄棒の長さを長くして。退廃さをひとひねりいれて」＋「この錫杖も参考にして」
//   添付＝骸華の過去の版（`.prev-gaika4〜7.mjs` の `armR` 24×64）で垂直に立てていた金棒。ドットを読み直して意匠をそのまま写した：
//   頭＝黒鉄の胴（左に鋼の光・中央に黒い溝）に金の乳（ち）が 2 列・上下を金の帯で締める＝梵鐘の乳を鉄に移した金砕棒（かなさいぼう）の頭。
//   棒を長くすると元の比率（頭は全長の 31%）では頭が細く沈むので、頭は幅 7→9・乳は 3→4 段（高さ 19→24）に伸ばし、柄も 3→4 ドットに太くした。
//   柄＝元は鋼＋蒼い神経光。指示どおり紅へ。長さは全長 62 →（第20案 124）→ 140 ドット。頂は世界 −86・下端 +54。
//   石突き＝金（幅 6・5 段）。柄の途中に金の帯 2 本（頭の下と石突きの上＝元の版と同じ位置）。
//   退廃のひとひねり＝**下から 2 段目の乳だけ深紅**（金の乳が整然と並ぶ中で一段だけ血が滲む＝壊れてはいないのに静かに間違っている）。
//   落とした稿＝頭の下に吊るす錫杖の遊環（等倍では左右に飛び出した耳にしか見えない）。比較は柄の紅の締め方 4 種（形は同じ）：A 明るい紅／B 深紅／C 深紅＋黒の影／D 明るい紅＋黒の影。
const ROD_DEFAULT = 'B';
const ROD_SPECS = { A: { cols: 'ARrr' }, B: { cols: 'RRrr' }, C: { cols: 'RRrk' }, D: { cols: 'ARrk' } };   // 柄の 4 列（左＝光の側 → 右）
export const GAIKA_ROD_LABELS = { A: 'A: BRIGHT CRIMSON', B: 'B: DEEP CRIMSON', C: 'C: DEEP + BLACK EDGE', D: 'D: BRIGHT + BLACK EDGE' };
const armL = (rod) => {
  const sp = ROD_SPECS[rod] || ROD_SPECS[ROD_DEFAULT], G = g(ARML_W, ARML_H);
  const CX = 10, FY = 96, H1 = 83, H0 = 60, B0 = -52, B1 = -57;                      // 柄の中心 x・拳の y／頭の上端と下端／柄の下端と石突きの下端（s＝拳から上）
  const put = (s, dx, ch) => P(G, CX + dx, FY - s, ch);
  const line = (s, x0, cols) => { for (let i = 0; i < cols.length; i++) if (cols[i] !== '.') put(s, x0 + i, cols[i]); };
  // --- 柄（紅）。元の版は鋼＋蒼い神経光・指示で紅へ
  for (let s = B0; s < H0; s++) line(s, -2, sp.cols);
  // --- 頭＝金砕棒。胴 'fmmmjjjj'／溝 'fmmkkjjj'／乳 2 列・4 段。下から 2 段目だけ深紅＝退廃のひとひねり
  const D = 'fmmmjjjj', V = 'fmmmkjjj', U = 'fWYmjWYj', L = 'fYymjYyj', u = 'fARmjARj', l = 'fRrmjRrj';
  const HEAD = [[-3, 'fmmjjj'], [-4, 'GGGGYYYYY'], [-4, D], [-4, D], [-4, U], [-4, L], [-4, V], [-4, V], [-4, D], [-4, U], [-4, L], [-4, V],
    [-4, V], [-4, D], [-4, u], [-4, l], [-4, V], [-4, V], [-4, D], [-4, U], [-4, L], [-4, D], [-4, 'GGGGYYYYY'], [-3, 'fmmjjj']];
  HEAD.forEach(([x0, cols], i) => line(H1 - i, x0, cols));
  // --- 柄の金の帯 2 本（頭の下と石突きの上）と石突き
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝梵鐘（鎮めの鐘）を金棒にし、その鐘の乳（ち）を鉄に移した頭で金砕棒（かなさいぼう）を鍛えた。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は金砕棒、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は金砕棒（かなさいぼう）＝鬼が持つ棒の形だが、頭は梵鐘の乳（ち）を鉄に移したもの。黒鉄の胴に金の乳が二列四段並び、上下を金の帯が締める。左手が鐘そのものを金棒にしているのに対し、こちらは鐘の肌だけを剥いで頭にした＝同じ「鎮めの道具を武器に変える」語彙。柄は血の色に塗られ、石突きは金。乳は整然と並ぶのに下から二段目だけが深紅に濡れている＝壊れてはいないのに静かに間違っている。'
