...
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (41,40)＝世界 (-17,-9)。肘 (29,54) → 拳 (18,46) が錫杖の柄を握る。錫杖は左へ傾いて立ち、頭（金の輪の中に自分の死面・深紅の遊環6つ）は冠より高い（世界 -49）
  ...
  const lw = (y) => 6.4 * Math.pow(Math.sin(Math.PI * (y - 2) / 23), 0.65);                                                // 頭の輪（尖った先・rows 2..25）
  ...
  const MX = ax(13.5), MY = 13.5, RX = 3.4, RY = 4.4, SH = 0.1;                                                        // 輪の中に自分の金の死面（穏やかに目を閉じる・右目から一筋の涙）
  for (let y = -RY; y <= RY; y += 0.25) for (let x = -RX; x <= RX; x += 0.25) {
  ...
  for (const s of [-1, 1]) ARC(G, MX + s * 1.5 + (-0.9) * SH, MY - 0.9, 0.8, 0.5, 200, 340, 'k');                          // 閉じた目
  for (let x = -0.7; x <= 0.7; x += 0.25) P(G, MX + x + 2.0 * SH, MY + 2.0, 'k');                                          // 口
  P(G, MX + 1.7, MY + 0.1, 'R'); P(G, MX + 1.8, MY + 0.9, 'R'); P(G, MX + 1.9, MY + 1.7, 'r');                             // 涙
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  ...
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は死面の錫杖、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の死面を掲げた錫杖（月牙と同じ高さ）。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は禍々しい月牙（深紅の縁・切っ先の光・後ろに影の月を従える）。後の左手は無く、右肩の上だけに翼の列が立つ＝左右非対称。冠の簾と帯の玉は紫＝最高位の色。'
