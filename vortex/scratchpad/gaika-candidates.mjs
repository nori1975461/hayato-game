...
//   ㉑左手の金棒を下に向ける：拳 (21,12)＝世界 (35,−1) が柄の上端を握り、真下へ垂らす。鐘は座の脇（世界 +23〜+42）で口が下＝吊られた鐘そのものの姿勢。
...
// =====================================================================
const ARMR_W = 34, ARMR_H = 58;
const ARM_R = (() => {
  // 骸華の左手（画面右）。肩 (3,4)＝世界 (17,-9)。肘 (12,17) → 拳 (21,12) が柄の上端を握り、金棒は真下へ垂れる（第12案：下に向ける）。
  // 頭＝梵鐘が下で口は下向き＝吊られた鐘そのものの姿勢（祀って鎮めるための鐘を引き抜いて棍棒にした）。鐘＝世界 +23〜+42・x 29〜41
  const G = g(ARMR_W, ARMR_H), sx = 21, B = 36;                                                                           // B＝竜頭の行
  for (let y = 5; y <= 7; y++) for (let x = sx - 2.5; x <= sx + 2.5; x += 0.5) P(G, x, y, y === 7 ? 'G' : 'Y');             // 石突（上端の金の帽）
  for (let y = 8; y <= B + 1; y++) for (let x = sx - 1.5; x <= sx + 1.5; x += 0.5) P(G, x, y, x < sx - 0.6 ? 'f' : x < sx + 0.6 ? 'm' : 'j');   // 柄（垂直・下へ）
  for (let y = 9; y <= B - 1; y += 0.5) P(G, sx + 1, y, 'c');                                                              // 柄の導管（神経光・連続線）
  for (const y of [19, 28]) for (let x = sx - 2; x <= sx + 2; x += 0.5) { P(G, x, y, 'G'); P(G, x, y + 1, 'Y'); }            // 金の環
  const bw = (t) => t < 1.5 ? 1.3 : t < 5 ? 2.4 + 2.2 * ((t - 1.5) / 3.5) : t < 12.5 ? 4.6 + 0.3 * ((t - 5) / 7.5) : 4.9 + 1.4 * ((t - 12.5) / 4);   // 梵鐘の輪郭（竜頭・丸い肩・胴・口へ広がる）
  for (let t = 0; t <= 16.5; t += 0.25) { const w = bw(t), y = B + t; for (let x = sx - w; x <= sx + w; x += 0.25) { const v = (x - sx) / w; P(G, x, y, t < 1.5 ? 'G' : t >= 14.5 ? (v < -0.2 ? 'G' : 'Y') : v < -0.45 ? 'f' : v < 0.15 ? 'm' : v < 0.65 ? 'j' : 'k'); } }
  for (const s of [-1, 1]) for (const t of [5.5, 8.5, 11.5]) bone(G, sx + s * 4.4, B + t, sx + s * 7.4, B + t - 1.4, 1.1, 0.4);   // 胴の両脇の骨の棘 3本ずつ（金棒の名残＝輪郭に出す）
  for (const dx of [-2.6, 2.6]) for (let t = 4.5; t <= 12.5; t += 0.25) for (let k = -0.75; k <= 0.75; k += 0.25) P(G, sx + dx + k, B + t, k < 0 ? 'R' : 'r');   // 袈裟襷（縦）＝深紅
  for (let x = sx - 4.6; x <= sx + 4.6; x += 0.25) for (let k = -0.75; k <= 0.75; k += 0.25) P(G, x, B + 8.5 + k, k < 0 ? 'R' : 'r');             // 袈裟襷（横）＝深紅
  for (let x = sx - 4.8; x <= sx + 4.8; x += 0.25) P(G, x, B + 12.9, 'k');                                                 // 帯の溝
  ELL(G, sx, B + 17.4, 5.8, 1.4, 'k');                                                                                    // 口の中（下から覗く空洞）
  DISC(G, sx, B + 17.6, 1.3, 'k'); DISC(G, sx, B + 17.6, 1.0, 'R'); P(G, sx - 0.4, B + 17.2, 'A');                         // 舌（深紅）
  arm(G, [[3, 4], [12, 17], [21, 12]], 3.6, 2.7);
  fist(G, 21, 11, 3.8, 3.4);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の死面を掲げた錫杖（月牙と同じ高さ）。左手（画面右）は金棒を真下へ垂らす＝頭は祀って鎮めるための梵鐘を引き抜いたもので、吊られた鐘の姿勢で座の脇に垂れる（深紅の袈裟襷・口の中に深紅の舌・胴の脇に骨の棘）。後の右手は大きな月牙、後の左手は芯が深紅の日輪。冠の簾と帯の玉は紫＝最高位の色。'
