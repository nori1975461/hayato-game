...
const PEDESTAL = (() => {
  ...
  for (let y = 12 + D; y <= 16 + D; y++) {                                                                                                          // 裳の襴＝胴の打ち合わせと同じ「深紅の地に金の菱」。衣装全体を一枚の法衣にまとめる。これより下は環と仰蓮に隠れる
    const hw = 20 + 12 * (y / (23 + D)), r = y - 12 - D;
    for (let x = Math.ceil(cx - hw); x <= Math.floor(cx + hw); x++) {
      const s = Math.abs(((x - cx + 406) % 12) - 6), dv = Math.abs(r - 2);
      let ch = 'r';                                                                                                                                 // 地は深紅のまま残す＝行ごと金で埋めると刺繍でなく金の棒になる
      if (r === 0 || r === 4) ch = 'y';                                                                                                             // 帯の上下の縁＝鈍い金の一筋
      else if (s + dv * 1.6 < 2.2) ch = s + dv * 1.6 < 1.1 ? 'G' : 'Y';                                                                              // 連珠＝12 ドットごとの小さな金の菱
      P(G, x, y, ch);
    }
  }
...
const TORSO = (() => {
  ...
  for (let y = 1; y < TORSO_H; y++) {
    ...
    for (let x = Math.ceil(cx - hw); x <= Math.floor(cx + hw); x++) {
      ...
      if (y >= LAP0 && y <= LAP1) {
        ...
        if (d >= 0 && d < 7.2) {                                                                                // 襴（らん）＝打ち合わせに沿う刺繍の帯。深紅の地に金の連珠。荘厳は縁飾りの幅で出る
          const w = d, s = (y - LAP0) % 7, dw = Math.abs(w - 3.6), ds = Math.abs(s - 3);
          ch = 'r';                                                                                             // 地は深紅。金の糸を縦に通すと珠がつながって「金の棒」になるので通さない
          if (w < 1 || w > 6.2) ch = 'y';                                                                       // 帯の両縁＝鈍い金の一筋
          if (dw * 1.3 + ds < 2.1) ch = dw * 1.3 + ds < 1.1 ? 'G' : 'Y';                                        // 連珠＝7 行ごとに離して並ぶ金の菱
          if (dw * 1.3 + ds < 0.35) ch = 'W';                                                                   // 菱の照り
        }
        if (d >= -1.4 && d < 0) ch = 'k';                                                                       // 帯が下前に落とす影
        if (d >= -2.6 && d < -1.4) ch = y < 26 ? 'R' : 'r';                                                     // その下から覗く深紅の裏地。襟の下に回すと赤い弧が「口」になったのでここへ移した
      }
      if (y >= 16 + E && y <= 19 + E) {                                                                         // 腰の帯＝縦の襴と直角に交わる横の襴。位置は「見える範囲の下端」＝これより下は蓮華座と環に隠れる
        const r = y - 16 - E, s = Math.abs(((x - cx + 405) % 10) - 5), dv = Math.abs(r - 1.5);
        ch = r === 0 || r === 3 ? 'y' : s + dv * 1.4 < 2.4 ? (s + dv * 1.4 < 1.2 ? 'G' : 'Y') : 'r';
      }
      P(G, x, y, ch);
    }
  }
  for (let y = 30; y <= 24 + E; y += 0.25) { const fx = cx + 7.5 + (y - 30) * 0.14; P(G, fx, y, 'k'); P(G, fx - 1, y, 'm'); }   // 留め具から落ちる布の襞（一本だけ）
  for (let gy = 12; gy <= 21 + E; gy += 8) for (let gx = -18; gx <= 18; gx += 10) {                             // 金襴の地文＝黒地に散る金の菱（刺繍）。輪に芯を入れると「目」になるので中は詰めず輪郭だけ
    const mx = cx + gx + (((gy - 12) / 8) % 2 ? 5 : 0);
    if (Math.abs(mx - lapX(gy)) < 9 || Math.abs(mx - cx) > hwAt(gy) - 3.5) continue;
    for (const [ox, oy] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) P(G, mx + ox, gy + oy, 'Y');
    for (const [ox, oy] of [[-2, 0], [2, 0], [0, -2], [0, 2]]) P(G, mx + ox, gy + oy, 'y');
  }
  ...
  for (const [dir, lit] of [[-1, true], [1, false]]) {                                                       // 肩＝板を重ねず一枚の鋭い仕立て（現代の外套の肩線）。外へ行くほど薄く尖る。牙だけは骨の語彙のまま残す
    for (let k = 0; k <= 11; k += 0.25) {
      ...
      if (k > 0.5 && k < 10) {                                                                                  // 肩の縁飾り＝金の二筋に深紅を挟む（胴の襴と同じ配色を肩へ回す）
        const yb = 10.4 - 4.4 * t * t;
        P(G, x, yb - 0.4, 'Y'); P(G, x, yb - 1.4, ((k * 2) | 0) % 3 ? 'r' : 'R'); P(G, x, yb - 2.3, 'y');
      }
    }
