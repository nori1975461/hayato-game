...
const GATLING = (() => {
  ...
  for (const s of [-1, 1]) {
    ...
    for (let y = Y(-31); y <= Y(-11); y += 0.25) for (let x = X(s * 10) ; s * (x - X(s * 37)) <= 0; x += s * 0.25) {
      const sg = q.map((p, i) => Math.sign(cr(p, q[(i + 1) % 4], x, y))); if (sg.some((v) => v && v !== sg.find((w) => w))) continue;
      const hem = Math.min(...[0, 1, 2].map((i) => Math.abs(cr(q[i], q[i + 1], x, y)) / Math.hypot(q[i + 1][0] - q[i][0], q[i + 1][1] - q[i][1])));   // 内側の辺（体側）を除く三辺からの距離
      P(G, x, y, hem >= 1.3 && hem < 2.3 ? 'R' : s * cr(q[0], q[2], x, y) < 0 ? 'j' : 'k');
    }
    LN(G, q[0][0], q[0][1], q[1][0], q[1][1], 'm');   // 第6稿（FB「兜と同じ黒・赤の刺繍」）＝外套と同じ黒（上面 j・下面 k・上の縁に m 一筋）に、三辺の縁から 1.3 入った深紅の刺繍一筋   // 第5稿（FB「大きすぎ・金をベースに赤の線」）＝金の地（上面 Y・下面 y・上の縁に G 一筋）に稜線の深紅一本
  }
  OUTLINE(G);
  return R(G);
})();
