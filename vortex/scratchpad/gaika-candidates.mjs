...
const ARML_W = 52, ARML_H = 128;
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (45,69)＝世界 (-17,-9)。肘 (33,83) → 拳 (16,75)＝世界 (-46,-3)。texture ＝ 世界 + (62, 78)
  // 第19案：ユーザー指示「棒にしよう。シンプルだが荘厳さと退廃さを併せ持った棒を。最強の闘神にふさわしい最凶の棒を」＋参考＝細い柄・両端に対称の飾り・等間隔の環と節・光る玉の杖 10 本。
  //   「鎮めの御柱（しずめのみはしら）」＝神を数える字が「柱」であるとおり、祀って鎮めるために立てられた柱そのものを、祀られる側の闘神が引き抜いて棒にした（梵鐘を金棒にしたのと同じ倒錯）。
  //   黒鉄の細い柄・鈍い金の環と深紅の玉の節・握りは深紅の巻き。両端は同じ形で柄と一直線＝金の座 → 硝子の玉 → 金の蓋 → 深紅の縁の黒鉄の鈷（両端とも突ける）。
  //   上の玉は蒼（封じた魂の色）・下の玉は深紅（血の色）＝天に魂・地に血。退廃は損傷でなく色（黒鉄・鈍い金・深紅）と倒錯で語る。
  //   棒は左へ 6.8° 傾いて立つ。以下は棒の軸に沿う座標 (s,t)：s＝拳から上へ・t＝左（光の側）へ。
  //   落とした稿＝金の鈷が閉じた籠に芯 → 等倍で「金のまぶたに青い瞳」＝縦の目／籠の胴に金の帯 → 金の塊に青い点二つ。学び＝幅 10 ドットの飾りに「籠と中身」は入らない。玉は囲まず、上下から挟む。
  const G = g(ARML_W, ARML_H);
  const FX = 16, FY = 75, UX = -0.1191, UY = -0.9929, NX = -0.9929, NY = 0.1191;
  const S = (s, t, ch) => P(G, FX + UX * s + NX * t, FY + UY * s + NY * t, ch);
  const gold = (t, hi) => (t > 1.0 ? (hi ? 'W' : 'G') : t > -1.0 ? (hi ? 'G' : 'Y') : (hi ? 'Y' : 'y'));
  // --- 柄（黒鉄・半幅 2.4）
  for (let s = -23.5; s <= 45.5; s += 0.2) for (let t = -2.4; t <= 2.4; t += 0.2) S(s, t, t > 1.6 ? 'f' : t > 0.4 ? 'm' : t > -1.6 ? 'j' : 'k');
  // --- 握り：深紅の巻き（黒い巻き目）と、上下の金の口
  for (let s = -8; s <= 8; s += 0.2) for (let t = -2.6; t <= 2.6; t += 0.2) S(s, t, (s + 8) % 2.4 < 0.7 ? 'k' : t > 0.4 ? 'R' : 'r');
  for (const s0 of [8, -9.6]) for (let s = s0; s <= s0 + 1.6; s += 0.2) for (let t = -3.1; t <= 3.1; t += 0.2) S(s, t, gold(t, false));
  // --- 節：金の環・深紅の玉・金の環
  const node = (sc) => { for (let d = -2.4; d <= 2.4; d += 0.15) { const bead = Math.abs(d) <= 1.3, w = bead ? 3.4 * Math.sqrt(1 - (d / 1.75) ** 2) : 3.0;
    for (let t = -w; t <= w; t += 0.2) S(sc + d, t, bead ? (t > 1.0 && d > 0 ? 'A' : t > -1.3 ? 'R' : 'r') : gold(t, false)); } };
  node(30); node(18); node(-16);
  // --- 両端の飾り（同じ形・柄と一直線）：金の座 → 硝子の玉 → 金の蓋 → 深紅の縁の黒鉄の鈷。dir＝+1 上 / −1 下
  const B = g(ARML_W, ARML_H);
  const finial = (s0, dir, ramp, glint) => {
    const Q = (q, t, ch, M = G) => P(M, FX + UX * (s0 + dir * q) + NX * t, FY + UY * (s0 + dir * q) + NY * t, ch);
    for (let q = 14.6; q <= 27.4; q += 0.12) { const w = 2.0 * (1 - Math.pow((q - 14.6) / 12.8, 1.15)) + 0.12; for (let t = -w; t <= w; t += 0.2) Q(q, t, t > 0 ? 'm' : 'j', B); }   // 先の鈷（縁取りは後で）
    for (const flip of [false, true]) for (let p = 0; p <= 3.4; p += 0.12) { const w = 2.7 + 1.3 * Math.pow(p / 3.4, 0.6);                                                  // 金の座と蓋（玉を上下から受ける杯）
      for (let t = -w; t <= w; t += 0.2) Q(flip ? 15.2 - p : p, t, p < 0.9 ? 'y' : gold(t * 0.8, p > 2.4)); }
    { const ox = FX + UX * (s0 + dir * 7.6), oy = FY + UY * (s0 + dir * 7.6); DISC(G, ox, oy, 5.7, 'k'); SPHERE(G, ox, oy, 5.0, ramp, [-0.5, -0.62, 0.6], 0.05); if (glint) { P(G, ox - 2, oy - 3, glint); P(G, ox - 3, oy - 2, glint); P(G, ox - 2, oy - 2, glint); } }                 // 硝子の玉（座より幅広＝丸い輪郭が出る）
    return [FX + UX * (s0 + dir * 27.4), FY + UY * (s0 + dir * 27.4)];
  };
  const tipT = finial(45, 1, ['k', 'q', 'q', 'Q', 'Q', 'P', 'P', 'N'], 'C');                                                                     // 上＝蒼硝子の玉（封じた魂の色）
  const tipB = finial(-23, -1, ['k', 'r', 'r', 'r', 'r', 'r', 'R', 'A'], null);                                                                    // 下＝深紅の玉（血の色）
  for (let y = 0; y < ARML_H; y++) for (let x = 0; x < ARML_W; x++) { if (B[y][x] === '.' || G[y][x] !== '.') continue;       // 先の鈷：外周 1 px を深紅に（蓮華の蓋の下は描かない）
    const open = [[1, 0], [-1, 0], [0, 1], [0, -1]].some(([a, b]) => !B[y + b] || B[y + b][x + a] === undefined || B[y + b][x + a] === '.');
    G[y][x] = open ? 'R' : B[y][x]; }
  P(G, tipT[0], tipT[1], 'A'); P(G, tipB[0], tipB[1], 'A');
  // --- 腕（棒の上に描く＝拳が握りを隠す）
  arm(G, [[45, 69], [33, 83], [16, 76]], 4.5, 3.4);
  armlet(G, [45, 69], [33, 83], 0.5, 4.6, true); armlet(G, [33, 83], [16, 76], 0.72, 3.9);                                    // 臂釧（深紅の玉）と腕釧＝金
  fist(G, 16, 75, 4.0, 3.6);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は「鎮めの御柱（しずめのみはしら）」の棒＝神を数える字が「柱」であるとおり、祀って鎮めるために立てられた柱そのものを、祀られる側の闘神が引き抜いて棒にした。黒鉄の細い柄に鈍い金の環と深紅の玉の節、握りは深紅の巻き。両端は同じ形で柄と一直線＝金の座・硝子の玉・金の蓋・深紅の縁の黒鉄の鈷（両端とも突ける）。上の玉は蒼（封じた魂の色）・下の玉は深紅（血の色）＝天に魂・地に血。'
...
function build(opts = {}) {   // opts.sword＝曲刀の中の左手（baseR）を付ける変種（第14案改の比較用・既定は無し）
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [45 / ARML_W, 69 / ARML_H] },
