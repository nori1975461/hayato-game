...
const ARML_W = 72, ARML_H = 116;
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (65,83)＝世界 (-17,-9)。肘 (53,97) → 拳 (37,89)＝世界 (-45,-3)。texture ＝ 世界 + (82, 92)
  // 第18案：「柄の先に頭を載せる棒」の軸そのものを捨てる（死面の錫杖・頭蓋・穂・大鎌・鉞＝同じ軸で 5 案つづけて不採用）。
  //   七支刀（しちしとう）＝神庫に祀られた祭祀の剣（斬るための剣ではない）を、祀られる側の悪神が武器として握る＝「鎮めの道具を武器に変える」の極点。
  //   幹の刃から六本の枝刃が左右互い違いに咲く（六本の腕の写し・鉄の華）。黒鉄の身・縁は深紅・幹に金象嵌の銘。七つの切っ先に蒼い鬼火が灯る＝剣であり、死者の灯明を立てる燭台。
  //   鍔は金の蓮華・柄頭は金の環頭（古墳の環頭大刀）に深紅の房。剣は左へ 15° 傾く（右の金棒は右下へ＝二つの武具が一本の斜線になる）。
  //   以下は剣の軸に沿う座標 (s,t)：s＝拳から切っ先へ・t＝左（光の側）へ。
  //   落とした稿＝倶利伽羅の降魔剣（骨の龍が巻きつき切っ先を呑む）→ 等倍で「白い帯を巻いた棒の先に白い顔と青い髪」＝また棒の先の顔になった。刃も細くて剣に見えなかった。
  const G = g(ARML_W, ARML_H);
  const FX = 37, FY = 89, UX = -0.2605, UY = -0.9655, NX = -0.9655, NY = 0.2605;
  const XY = (s, t) => [FX + UX * s + NX * t, FY + UY * s + NY * t];
  const S = (s, t, ch) => P(G, FX + UX * s + NX * t, FY + UY * s + NY * t, ch);
  const BR = [[16, 1], [24, -1], [32, 1], [40, -1], [48, 1], [56, -1]];                                                      // 枝刃の根 (s0, 側)
  const bez = (s0, sd, v) => { const r = sd > 0 ? 11 : 9.5, a = [s0, sd * 2.8], b = [s0 + 1.8, sd * (r - 1.2)], c = [s0 + 9, sd * r], w = 1 - v;   // 枝刃は外へ張り出して上へ反る（左 11・右 9.5＝影の月を避ける）
    return [w * w * a[0] + 2 * v * w * b[0] + v * v * c[0], w * w * a[1] + 2 * v * w * b[1] + v * v * c[1], 2 * w * (b[0] - a[0]) + 2 * v * (c[0] - b[0]), 2 * w * (b[1] - a[1]) + 2 * v * (c[1] - b[1])]; };
  const TIP = 76;
  // --- 七つの鬼火（刃の後ろ・火は剣の傾きに関わらず真上へ上がる）
  for (const [s0, sd] of BR) { const [ts, tt] = bez(s0, sd, 1), [x, y] = XY(ts, tt); flame(G, x, y + 0.5, x + sd * -0.5, y - 5.6, 1.75, sd * 0.6); }
  { const [x, y] = XY(TIP, 0); flame(G, x, y + 0.5, x + 0.8, y - 9.5, 2.5, -0.9); }
  // --- 刃（別の版に描いてから、外周 1 px を深紅の刃にする＝七支の輪郭が一筆の赤で縁取られる）
  const B = g(ARML_W, ARML_H);
  const SB = (s, t, ch) => P(B, FX + UX * s + NX * t, FY + UY * s + NY * t, ch);
  for (const [s0, sd] of BR) for (let v = 0; v <= 1; v += 0.003) {
    const [cs, ct, ds, dt] = bez(s0, sd, v), L = Math.hypot(ds, dt), ns = -dt / L, nt = ds / L, w = 0.25 + 2.5 * (1 - Math.pow(v, 1.8));
    for (let k = -w; k <= w; k += 0.2) { const lit = (UX * ns + NX * nt) * k * -0.63 + (UY * ns + NY * nt) * k * -0.78 > 0; SB(cs + ns * k, ct + nt * k, lit ? 'm' : 'j'); }
  }
  for (let s = 10.2; s <= TIP; s += 0.15) { const w = s < 63 ? 3.9 : 3.9 * (1 - Math.pow((s - 63) / (TIP - 63), 1.3)) + 0.15;
    for (let t = -w; t <= w; t += 0.2) SB(s, t, Math.abs(t) < 0.62 && s > 13 && s < 65 && (s - 13) % 3.2 < 2.2 ? (t > 0 ? 'G' : 'Y') : t > 0 ? 'm' : 'j'); }   // 幹：金象嵌の銘（金の短冊が縦に並ぶ）
  for (let y = 0; y < ARML_H; y++) for (let x = 0; x < ARML_W; x++) { if (B[y][x] === '.') continue;
    const open = [[1, 0], [-1, 0], [0, 1], [0, -1]].some(([a, b]) => !B[y + b] || B[y + b][x + a] === undefined || B[y + b][x + a] === '.');
    G[y][x] = open ? 'R' : B[y][x]; }
  for (const [s0, sd] of BR) { const [ts, tt] = bez(s0, sd, 1), [x, y] = XY(ts, tt); P(G, x, y, 'A'); }                       // 切っ先の光
  { const [x, y] = XY(TIP, 0); P(G, x, y, 'A'); }
  // --- 握りの上と金の環
  for (let s = 2.5; s <= 6.4; s += 0.2) for (let t = -1.9; t <= 1.9; t += 0.2) S(s, t, s > 5.2 ? (t > 0 ? 'G' : 'Y') : t > 0.6 ? 'm' : 'j');
  // --- 蓮華の鍔：上へ開く金の蓮弁 3 枚（半幅 2.8 → 8・等倍で読める大きさ）・下は暗い金の帯・芯に深紅の玉
  for (let s = 6; s <= 11.6; s += 0.15) { const w = 2.8 + 5.2 * Math.pow((s - 6) / 5.5, 0.7); for (let t = -w; t <= w; t += 0.2) {
    const c = Math.abs(Math.cos(Math.PI * t / 5.4)), top = 9.8 + 1.7 * c; if (s > top) continue;
    const tip = s > top - 0.9, seam = c < 0.17 && s > 7.6;
    S(s, t, s < 7.3 ? 'y' : seam ? 'y' : t > 2.2 ? (tip ? 'W' : 'G') : t > -2.4 ? (tip ? 'G' : 'Y') : (tip ? 'Y' : 'y'));
  } }
  { const [x, y] = XY(8.6, 0); DISC(G, x, y, 1.45, 'k'); DISC(G, x, y, 1.0, 'R'); P(G, x - 0.5, y - 0.5, 'A'); }
  // --- 拳の下：黒い握り → 金の口 → 金の環頭 → 深紅の房（真下へ垂れる）
  for (let s = -7; s <= -2.5; s += 0.2) for (let t = -1.9; t <= 1.9; t += 0.2) S(s, t, s < -6 ? (t > 0 ? 'G' : 'Y') : t > 0.6 ? 'm' : 'j');
  { const [cx, cy] = XY(-10.8, 0);
    for (let y = cy + 3; y <= cy + 9.5; y += 0.25) for (let x = -0.6; x <= 0.6; x += 0.25) P(G, cx + 0.4 + x, y, x < 0 ? 'R' : 'r');
    for (let y = -2.6; y <= 2.6; y += 0.25) for (let x = -1.6; x <= 1.6; x += 0.25) { if ((x / 1.6) ** 2 + (y / 2.6) ** 2 > 1) continue; P(G, cx + 0.4 + x, cy + 11 + y, x < -0.5 ? 'A' : x < 0.6 ? 'R' : 'r'); }
    for (let y = -3.6; y <= 3.6; y += 0.25) for (let x = -3.6; x <= 3.6; x += 0.25) { const d = Math.hypot(x, y); if (d > 3.6 || d < 1.7) continue; const v = (x * -0.63 + y * -0.78) / 3.6; P(G, cx + x, cy + y, v > 0.5 ? 'W' : v > 0 ? 'G' : v > -0.5 ? 'Y' : 'y'); } }
  // --- 腕（剣の上に描く＝拳が握りを隠す）
  arm(G, [[65, 83], [53, 97], [37, 90]], 4.5, 3.4);
  armlet(G, [65, 83], [53, 97], 0.5, 4.6, true); armlet(G, [53, 97], [37, 90], 0.72, 3.9);                                 // 臂釧（深紅の玉）と腕釧＝金
  fist(G, 37, 89, 4.0, 3.6);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝神庫に祀られた祭祀の七支刀を斬るために握り、梵鐘（鎮めの鐘）を金棒にした。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は七つの鬼火を灯す七支刀、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は「七支刀（しちしとう）」＝神庫に祀られた祭祀の剣（斬るための剣ではない）を、祀られる側の悪神が武器として握る。黒鉄の幹から六本の枝刃が左右互い違いに咲き（六本の腕の写し・鉄の華）、輪郭は一筆の深紅、幹に金象嵌の銘、七つの切っ先に蒼い鬼火＝剣であり死者の灯明を立てる燭台。鍔は金の蓮華・柄頭は金の環頭に深紅の房。'
...
function build(opts = {}) {   // opts.sword＝曲刀の中の左手（baseR）を付ける変種（第14案改の比較用・既定は無し）
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [65 / ARML_W, 83 / ARML_H] },
