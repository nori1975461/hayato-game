...
const TORSO = (() => {
  ...
  const lapX = (y) => cx + 10.5 - 22 * (y - LAP0) / (LAP1 - LAP0);                                             // 左前＝上に来るのは骸華の左の身頃。縁は襟元の右から腰の左へ下る
  for (let y = 1; y < TORSO_H; y++) {
    ...
    for (let x = Math.ceil(cx - hw); x <= Math.floor(cx + hw); x++) {
      ...
      if (y >= LAP0 && y <= LAP1) {
        ...
        if (d >= -2.4 && d < -1.3) ch = y < 26 ? 'R' : 'r';                                                     // 上前の下から覗く深紅の裏地。襟の下に回すと赤い弧が「口」になったのでここへ移した
        if (d >= -1.3 && d < 0) ch = 'k';                                                                       // 上前の縁が下前に落とす影
        if (d >= 0 && d < 1.3) ch = y < 24 ? 'G' : 'Y';                                                         // 縁取りは鈍い金の一本線だけ（襟元は明るく腰へ鈍る）
      }
  ...
  for (const [dir, lit] of [[-1, true], [1, false]]) {                                                       // 肩＝板を重ねず一枚の鋭い仕立て（現代の外套の肩線）。外へ行くほど薄く尖る。牙だけは骨の語彙のまま残す
    for (let k = 0; k <= 11; k += 0.25) {
      ...
      for (let y = yTop; y <= yBot; y += 0.25) {
        ...
        P(G, x, y, v < 0.15 ? 'k' : v < 0.44 ? (lit ? 's' : 'f') : v < 0.8 ? (lit ? 'f' : 'm') : (lit ? 'm' : 'j'));
      }
    }
    bone(G, cx + dir * 18, 5, cx + dir * 21, 1.5, 1.6, 0.8); P(G, cx + dir * 21.5, 1, 'n');
    for (let x = cx + dir * 11; dir > 0 ? x <= cx + 19 : x >= cx - 19; x += dir) P(G, x, 10.2, 'Y');            // 肩の縁を鈍い金で一筋だけ縫う
  }
