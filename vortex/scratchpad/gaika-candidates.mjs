...
//   ㊵**既定の先端を D（宝塔）へ**。合わせて先端を 11→15 ドットに伸ばし、宝形の屋根を四段（4→6→8→10）に開いて軒をいちばん広くした＝等倍で「金の郵便受け」だったものが塔の影になる。軒の両端に**深紅の風鐸（ふうたく）**＝先端に三色目が入る。塔身の中は黒のまま＝円光の中が空なのと同じ構造。
//   ㊵**縦横比は 0.74（横/縦）を採用**。根拠＝①ユーザーの添付画像（963×1258）が 0.766 ②他の三柱は 大聖堂 1.650／腐蝕の玉座 1.159／軌道神核 1.315／マオウレクス 1.053 とすべて横長＝骸華だけ縦長にすると四神柱で唯一の闘神だと一目でわかる ③改訂前は 0.993（棒込み）でも**棒を除くと 1.164 の横長**＝棒だけが縦の数字を作っていた。
//      実測＝塗り 521×701 px（124×167 units）＝**棒込み 0.743・本体のみ 0.849**。縦を伸ばした内訳は五つ：環 142×60→122×68（狭く高く・rig oy −10）／光背の骸を上へ振る／台座 34→58 で**須弥壇（上框・朱の一筋・連子窓の三面・下框）**を足す／冕冠 CROWN_H 26→34（角と中心の鉾を伸ばす・rig oy −48）／錫杖を基壇の床まで伸ばして**杖が床に立つ**（ARML_H 160→186）。
//      基壇の窓は三稿めで決着＝半円のアーチは「建物の窓」・火灯窓は「西洋のテント（大聖堂と被る）」・**連子窓（縦格子）だけが寺社に見え、同時に檻に見える**＝「祀って鎮める＝封じ込める」を形が言う。学び＝**曲線の開口は縮尺が小さいと国籍を失う。縦棒の繰り返しは失わない**。
//      横幅は一切広げていない（世界 x −62〜62 のまま）＝「横に広い」は幅の問題でなく**縦が足りない**問題だった。学び＝比率の指摘は先に「どちらが足りないか」を測る。棒のような細長い付属物は数字を膨らませるので、本体だけの比も必ず併記する。
//
...
function mandorla(variant) {
  ...
  const husks = variant === 'A' ? [[62, 47, 17], [-28, 54, 14], [-62, 47, 17]] : [[30, 50, 14], [-30, 50, 14]];   // [角度, 長さ, 根元の半径]＝A の外側2枚は横へ張り出す。第14案改：内側右（+30°＝左肩の上）は削除＝片側だけの翼
...
const PEDESTAL = (() => {
  ...
  for (const x of [16, 91]) { DISC(G, x, 30.6, 1.5, 'k'); DISC(G, x, 30.6, 1.1, 'R'); P(G, x - 0.4, 30.2, 'A'); }
  for (let x = 4; x <= 103; x++) { P(G, x, 34, 'Y'); P(G, x, 35, 'y'); }                                                                                          // 基壇の上框（うわがまち）＝金の一筋で受ける
  for (let x = 8; x <= 99; x++) P(G, x, 36, 'r');                                                                                                                 // 朱の一筋＝壇の縁。鮮やかな朱でなく沈んだ血の紅
  for (let y = 37; y <= 51; y++) for (let x = 10; x <= 97; x++) P(G, x, y, x < 13 ? 'm' : 'j');                                                                    // 基壇の束（つか）＝黒鉄。明るくすると床の帯に見える
  for (const gx of [31, 53.5, 76]) {                                                                                                                              // 連子窓（れんじまど）＝寺社の縦格子。闇に鈍い金の縦棒が三面。祀って鎮める＝封じ込める檻でもある
    const x0 = Math.round(gx - 7), x1 = Math.round(gx + 7), Y0 = 38, Y1 = 51;
    for (let y = Y0; y <= Y1; y++) for (let x = x0; x <= x1; x++) P(G, x, y, 'k');                                                                                 // 中は闇＝円光の中が空なのと同じ構造
    for (let x = x0 + 3; x <= x1 - 2; x += 4) for (let y = Y0 + 1; y < Y1; y++) P(G, x, y, y < Y0 + 2 ? 'Y' : 'y');                                                // 連子＝縦棒。上だけ光を受ける
    for (let y = Y0; y <= Y1; y++) { P(G, x0, y, 'Y'); P(G, x1, y, 'y'); }
    for (let x = x0; x <= x1; x++) { P(G, x, Y0, 'Y'); P(G, x, Y1, 'y'); }                                                                                        // 窓枠
  }
  for (let x = 4; x <= 103; x++) { P(G, x, 52, 'Y'); P(G, x, 53, 'y'); }                                                                                          // 下框の金の一筋（上框と同じ幅＝棚に見せない）
  for (let y = 54; y <= 57; y++) for (let x = 0; x <= 107; x++) P(G, x, y, y < 56 ? 'j' : 'k');                                                                    // 最下の框
  for (let x = 6; x <= 101; x += 13) P(G, x, 54, 'Y');                                  // 鎖の底の深紅の玉
  OUTLINE(G);
  return R(G);
})();
  ...
  for (const [dir, lit] of [[-1, true], [1, false]]) {                                                       // 肩当て（2層）＋牙
    for (let layer = 0; layer < 2; layer++) {
      ...
      for (let y = y0; y < y0 + 3; y++) for (let x = Math.min(x0, x1); x <= Math.max(x0, x1); x++) {
        let ch = y === y0 ? 'k' : y === y0 + 1 ? (lit ? 's' : 'f') : (lit ? 'f' : 'm');                       // 肩当ては骨のまま（赤糸縅は胸だけ）
...
// =====================================================================
const CROWN_W = 40, CROWN_H = 34;
const CROWN = (() => {
  const G = g(CROWN_W, CROWN_H), cx = 19.5, B = 21;                                                   // 第23案：全体を縦長にするため角と尖りを伸ばした（帯の世界位置は据え置き＝rig の oy を -48 へ）
  for (const d of [1, -1]) {
    const X = (x) => cx + d * (x - cx);
    bone(G, X(29.5), B + 0.5, X(34.2), B - 7, 2.3, 1.8); bone(G, X(34.2), B - 7, X(37.2), B - 14, 1.8, 1.2); bone(G, X(37.2), B - 14, X(38.4), B - 18.4, 1.2, 0.6); P(G, X(38.6), B - 18.9, 'n');
    P(G, X(33), B - 4.5, 'k'); P(G, X(35.6), B - 6.4, 'k'); P(G, X(36.2), B - 11, 'k'); P(G, X(38), B - 12.4, 'k');
    for (const [x, y] of [[32, B - 2.5], [35.6, B - 9]]) { P(G, X(x), y, 'Y'); P(G, X(x + 1), y - 1, 'Y'); }             // 角の金の環
  }
  ...
  for (let y = B - 14; y <= B - 1; y++) { const hw = y < B - 10 ? 0.5 : 1.5; for (let x = Math.ceil(cx - hw); x <= Math.floor(cx + hw); x++) P(G, x, y, x < cx ? (y < B - 6 ? 'W' : 'G') : 'Y'); }
  P(G, 19, B - 14, 'W'); P(G, 20, B - 14, 'W');
  for (const [c0, lit] of [[14.5, true], [25.5, false]]) {
    for (let y = B - 8; y <= B - 1; y++) { const hw = 0.5 + (y - (B - 8)) * 0.34; for (let x = Math.ceil(c0 - hw); x <= Math.floor(c0 + hw); x++) P(G, x, y, x < c0 ? (lit ? 'W' : 'G') : (lit ? 'G' : 'Y')); }
  }
...
const armL = (rod) => {
  ...
  const CX = 10, FY = 96, TOP = 83, B0 = -76, B1 = -81;                              // 柄の中心 x・拳の y／棒の頂／柄の下端と石突きの下端（s＝拳から上）。第23案で棒を基壇の下端（世界 +78）まで伸ばした＝床に立てる杖
...
const WING_L = (() => {                                                                                                  // 後の右手（画面左上）＝禍々しい月牙：主の月（r 11.5・蒼硝子・縁は深紅＝血の刃・切っ先に光）と、後ろ上に従う影の月（紫）。影の月は錫杖の輪より上に置く（重ねない）。頂は世界 −68.6＝最上部
  const G = g(WINGL_W, WINGL_H);
  arm(G, [[38, 53], [14, 38], [20, 28]], 3.2, 2.4); fist(G, 21, 27, 3.0, 2.6); dimBone(G);
  crescent(G, 11, 9.5, 8.2, 4.0, -1.8, 7.0, { rim: 'V', rimW: 1.2, mid: 'v', dark: 'v' });                                // 影の月（後ろ・小さい・紫の身に薄紫の縁）
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  ...
  + '下半身は裳懸座＝黒鉄の裳が膝の下から牙の座の前を四段で流れ落ち、その下に蓮華座の段（仰蓮・段・反花に金の鎖の垂れ飾り・框に配管）。その下に須弥壇（しゅみだん）＝上框の金の一筋・朱の一筋・連子窓が三面・下框が床まで続く。足は無い。';
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は錫杖＝本来は鳴らして獣を追い払う音の杖で、殺さないための道具。柄は磨いた黒曜石で、鋭い照りが一筋だけ走る。頂は宝塔（ほうとう）＝舎利を納めるための塔で、宝形の屋根が四段に開き、軒の両端に深紅の風鐸（ふうたく）が下がる。塔身の中は塗り潰した黒＝円光の中が空なのと同じで、納められるべき舎利は無い。獣を退けるための杖を人の背丈を越える長さに伸ばし、石突きは基壇の床を踏んでいる＝杖でなく柱として立っている。'
  + '姿の比は縦長＝塗り 124×167 units（横/縦 0.74・棒を除くと 0.85）。他の三柱がすべて横に広いのに対し骸華だけが縦に立つ＝四神柱でただ一体の闘神。'
...
// =====================================================================
const RING_W = 122, RING_H = 68;
const RINGS = [[59, 22, 25, 'e', 'E', true], [52, 18, 14, 'Y', 'W', true], [45, 15, 4, 'v', 'V', false]];                     // [rx, ry, 中心 y（世界）, 影, 光, 錠]＝外から玉座の緑青（座の高さ）・大聖堂の金（裳の暗がりに映える）・軌道神核の紫（帯）
...
const ringsSprite = (front) => {
  const G = g(RING_W, RING_H), cx = 60.5;
