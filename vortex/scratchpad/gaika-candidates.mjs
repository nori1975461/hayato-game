...
const ARML_W = 62, ARML_H = 113;
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (57,61)＝世界 (-17,-9)。肘 (45,75) → 拳 (34,67) が錫杖の柄を握る。柄は左へ傾いて立つ
  // 第17案：頭は「鉞（まさかり）」＝獣の口の黒鉄の口金（金の帯・金の牙・深紅の玉）が蒼白い半月の刃（深紅の血溝）を咥え、口金の下に深紅の遊環 2 対。
  //   殺さないための音の杖（錫杖）に処刑の斧を挿げた＝戒を捨てた悪神の証。梵鐘を金棒にしたのと同じ倒錯。鉞は王権と処刑の印＝荘厳と禍々しさの同居。
  //   落とした稿＝第14案までの金の輪＋宝珠（顔に見えない）・第15案の頭蓋（武具のかっこよさが消えた）・第16案の蒼硝子の穂（うさぎの耳）・第16案改の大鎌（金の輪と合わない）。
  //   上に T=13・左に X=16 広げてある（以下の座標は旧座標 + T / + X。rows 0〜17 は未使用）
  const T = 13, X = 16;
  const G = g(ARML_W, ARML_H);
  const ax = (y) => X + 14 - (44 + T - y) * 0.11;                                                                          // 柄の軸（左へ傾く）
  for (let y = 23 + T; y <= 93 + T; y++) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) P(G, x, y, x < c - 1.2 ? 'f' : x < c - 0.2 ? 'm' : x < c + 1.2 ? 'j' : 'k'); }   // 柄（口金の下端まで）
  for (const y of [58 + T, 74 + T, 88 + T]) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) { P(G, x, y, 'G'); P(G, x, y + 1, 'Y'); } }   // 金の環
  for (let y = 94 + T; y <= 99 + T; y++) { const c = ax(y); for (let x = c - 2.5; x <= c + 2.5; x += 0.5) P(G, x, y, y === 94 + T ? 'G' : 'Y'); }   // 石突
  // ===== 第17案：錫杖鉞（しゃくじょうまさかり）＝錫杖の柄に鉞の頭を挿げる。参考＝獣頭の口金が半月の刃を咥える儀礼の鉞（王権＝荘厳・処刑＝禍々しさ） =====
  const SX = ax(16 + T), SY = 16 + T;                                                                                    // 口金の芯＝世界 (−47,−41)
  { // 刃：髭斧の扇。主の楕円 E1（芯 (−9.5,0.5)・rx 10.5・ry 11.5）を口金の左（lx ≤ −3.4）で切り、上は左へ僅かに上がる平らな峰、口金の下は楕円 E2（芯 (−5,12.5)・rx 7・ry 5）で抉って顎の切れ込み
    //   蒼白い骨鉄 's'・外の弧＝深紅の刃 1 筋＋白の鎬（しのぎ）1 筋・口金の脇（背）と顎の下は 'f'。落とした稿＝左右対称の半楕円＋同心の深紅の弧 → 等倍で「白い丸に赤い輪」＝目に見えた
    for (let ly = -12; ly <= 12.5; ly += 0.25) for (let lx = -21; lx <= -3.4; lx += 0.25) {
      const r1 = Math.hypot((lx + 9.5) / 10.5, (ly - 0.5) / 11.5); if (r1 > 1) continue;
      if (ly < -8 - 0.18 * (-3.4 - lx)) continue;
      if (Math.hypot((lx + 5) / 7, (ly - 12.5) / 5) < 1) continue;
      const top = -8 - 0.18 * (-3.4 - lx);
      P(G, SX + lx, SY + ly, ly < top + 1.0 && r1 < 0.97 ? (lx < -11 ? 'G' : 'Y') : r1 > 0.92 ? 'R' : r1 > 0.84 ? 'n' : lx > -7.5 || ly > 8 ? 'f' : 's');   // 峰に金の一筋（荘厳）
    }
    P(G, SX - 12.5, SY + 11.6, 'A');                                                                                     // 顎の切っ先の光
    for (let ly = -7.5; ly <= 7.5; ly += 0.25) P(G, SX - 3.6, SY + ly, 'k');                                               // 刃と口金の境の黒い線
  }
  for (let ly = -7; ly <= 7; ly += 0.25) for (let lx = -3.4; lx <= 3.4; lx += 0.25) {                                       // 口金＝獣の口（黒鉄・上下の端に金の帯）
    const band = Math.abs(ly + 6.2) < 0.75 || Math.abs(ly - 6.2) < 0.75;
    P(G, SX + lx, SY + ly, band ? (lx < 0 ? 'G' : 'Y') : lx < -1.8 ? 'f' : lx < 0.6 ? 'm' : 'j');
  }
  for (const s of [-1, 1]) {                                                                                             // 金の牙 2 本（黒縁）＝口が刃の背を咥える
    for (let t = 0; t <= 3.4; t += 0.25) { const h = 1.5 * (1 - t / 3.4); for (let k = -h; k <= h; k += 0.25) P(G, SX - 3.4 - t, SY + s * 3 + k, 'k'); }
    for (let t = 0; t <= 2.8; t += 0.25) { const h = 1.0 * (1 - t / 2.8); for (let k = -h; k <= h; k += 0.25) P(G, SX - 3.4 - t, SY + s * 3 + k, 'G'); }
  }
  DISC(G, SX, SY, 1.9, 'k'); DISC(G, SX, SY, 1.4, 'R'); P(G, SX - 0.5, SY - 0.5, 'A');                                    // 口金の芯の深紅の玉
  for (let ly = -10; ly <= -7; ly += 0.25) { const w = 2.4 * Math.sqrt(1 - 0.85 * ((ly + 7) / 3) ** 2); for (let lx = -w; lx <= w; lx += 0.25) P(G, SX + lx, SY + ly, Math.abs(ly + 8.4) < 0.5 ? 'Y' : lx < -1 ? 'f' : lx < 0.6 ? 'm' : 'j'); }   // 口金の頭（丸い黒鉄・金の帯）
  for (const [ly, s] of [[9.5, -1], [9.5, 1], [13, -1], [13, 1]]) { const y = SY + ly, c = ax(y) + s * 3.2; DISC(G, c, y, 1.7, 'k'); DISC(G, c, y, 1.25, 'R'); P(G, c, y, 'k'); P(G, c - 0.5, y - 0.5, 'A'); }   // 深紅の遊環 2 対（口金の下＝錫杖の名残・参考の輪）
  arm(G, [[41 + X, 48 + T], [29 + X, 62 + T], [18 + X, 54 + T]], 4.5, 3.4);                                                // 第13案：腕を 1/4 太く（3.6/2.7 → 4.5/3.4・胴 46 の 1/5）
  armlet(G, [41 + X, 48 + T], [29 + X, 62 + T], 0.5, 4.6, true); armlet(G, [29 + X, 62 + T], [18 + X, 54 + T], 0.72, 3.9);   // 臂釧（深紅の玉）と腕釧＝金
  fist(G, 18 + X, 53 + T, 4.0, 3.6);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝錫杖（獣を追い払う音の杖）に処刑の鉞を挿げ、梵鐘（鎮めの鐘）を金棒にした。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は鉞の頭を挿げた錫杖、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は錫杖の柄に鉞（まさかり）の頭を挿げた「錫杖鉞」＝獣の口の黒鉄の口金（金の牙・深紅の玉）が蒼白い半月の刃（深紅の血溝）を咥え、口金の下に深紅の遊環。殺さないための音の杖に処刑の斧を継いだ戒破りの武具で、鉞＝王権と処刑の印＝荘厳と禍々しさの同居。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は禍々しい月牙（深紅の縁・切っ先の光・後ろに影の月を従える）。後の左手は無く、右肩の上だけに翼の列が立つ＝左右非対称。冠の簾と帯の玉は紫＝最高位の色。'
...
function build(opts = {}) {   // opts.sword＝曲刀の中の左手（baseR）を付ける変種（第14案改の比較用・既定は無し）
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [57 / ARML_W, 61 / ARML_H] },
