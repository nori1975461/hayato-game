...
const HEADS = {
  ...
  D: [[-1, 'GG'], [-1, 'GY'], [-2, '.YY.'], [-3, 'YYYYYY'], [-4, 'GYYYYYYG'], [-3, 'GmjjmY'], [-3, 'GmjjmY'], [-3, 'GmjjmY'], [-4, 'YYYYYYYY'], [-4, 'GmjjjjmY'], [-4, 'YYYYYYYY']] };   // 宝塔＝宝形の屋根に円い胴
const armL = (rod) => {
  const G = g(ARML_W, ARML_H), key = HEADS[rod] ? rod : ROD_DEFAULT, H = HEADS[key];
  const CX = 10, FY = 96, TOP = 83, B0 = -52, B1 = -57;                              // 柄の中心 x・拳の y／棒の頂／柄の下端と石突きの下端（s＝拳から上）
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は錫杖＝本来は鳴らして獣を追い払う音の杖で、殺さないための道具。柄は磨いた黒曜石で、鋭い照りが一筋だけ走る。頂の金の輪の中は塗り潰した黒＝円光の中が空なのと同じで、そこに祀られるべき仏は居ない。輪の下に深紅の遊環が二対下がり、振れば血の色の音が鳴る。獣を退けるための杖を、人の背丈を越える長さに伸ばして握っている。'
  + '深紅は封印の内側から滲む神の色＝胸の赤糸縅・円光の宝珠・錫杖の遊環・環の錠・月牙の玉・死面の涙・胎の心臓・盃の流れ。紙垂は骨白＝封印の紙は清い。';
