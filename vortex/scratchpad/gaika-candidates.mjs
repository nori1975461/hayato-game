...
const WING_R = (() => {
  ...
  DISC(G, 15, 3.6, 1.5, 'R');                                                                                            // 日輪の芯は深紅（月牙の玉と対）
  OUTLINE(G);
  return R(G);
})();
const WINGL_W = 40, WINGL_H = 46;
const WING_L = (() => {                                                                                                  // 後の右手（画面左上）＝月牙：大きな三日月の刃（r 9.6・蒼硝子・外の縁が光る・深紅の玉）。日輪と非対称＝最上部
  const G = g(WINGL_W, WINGL_H);
  arm(G, [[35, 43], [12, 30], [17, 19]], 3.2, 2.4); fist(G, 18, 17, 3.0, 2.6); dimBone(G);
  DISC(G, 18, 9.5, 10.4, 'k'); DISC(G, 18, 9.5, 9.6, 'N'); DISC(G, 18, 9.5, 8.0, 'P');
  for (let a = 150; a <= 330; a += 0.3) { const t = a * Math.PI / 180; for (let r = 8.6; r <= 9.7; r += 0.25) P(G, 18 + Math.cos(t) * r, 9.5 + Math.sin(t) * r, 'C'); }   // 外の刃の光
  DISC(G, 22.5, 7.5, 8.0, '.'); ARC(G, 22.5, 7.5, 8.0, 8.0, 100, 262, 'k');                                              // 欠け（内側の弧）
  DISC(G, 12.5, 12, 2.2, 'k'); DISC(G, 12.5, 12, 1.6, 'R'); P(G, 12, 11.5, 'A');                                        // 深紅の玉
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の死面を掲げた錫杖（月牙と同じ高さ）。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は大きな月牙、後の左手は芯が深紅の日輪。冠の簾と帯の玉は紫＝最高位の色。'
...
function build() {
  ...
  const rig = [
    ...
    { role: 'wingL', tex: 'wingL', ox: -10, oy: -16, origin: [35 / WINGL_W, 43 / WINGL_H] },
