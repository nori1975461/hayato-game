...
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (57,61)＝世界 (-17,-9)。肘 (45,75) → 拳 (34,67) が錫杖の柄を握る。錫杖は左へ傾いて立ち、頭（尖った金の輪の中に金の宝珠・深紅の遊環6つ）は冠より高い
  // 第16案改：輪の頂を抱く黒鉄の口金から左へ「大鎌の刃」を出す＝獣を追い払う音の杖（殺さないための道具）に死神の鎌を継いだ＝戒を捨てた悪神の証。梵鐘を金棒にしたのと同じ倒錯。
  //   刃＝黒鉄（峰は骨鉄の光・刃は深紅・切っ先に光）・根元の厚さ 6.8・左へ 19 下へ 10 曲がる（世界 x −72 まで・影の月とは反対側）。顔は載せない（輪の中は金の宝珠）。
  //   落とした稿（第16案）＝頂から上へ木の葉形の蒼硝子の穂 → 細い首・両側が丸い・桃色の縁で「うさぎの耳」に見え、斬れば折れる弱さしか無かった。
  //   上に T=13・左に X=16 広げてある（以下の座標は旧座標 + T / + X）
  const T = 13, X = 16;
  const G = g(ARML_W, ARML_H);
  const ax = (y) => X + 14 - (44 + T - y) * 0.11;                                                                          // 柄の軸（左へ傾く）
  for (let y = 26 + T; y <= 93 + T; y++) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) P(G, x, y, x < c - 1.2 ? 'f' : x < c - 0.2 ? 'm' : x < c + 1.2 ? 'j' : 'k'); }   // 柄
  ...
  const lw = (y) => 6.4 * Math.pow(Math.sin(Math.PI * (y - 2 - T) / 23), 0.65);                                            // 頭の輪（尖った先・rows 2+T..25+T）＝第14案までの形
  for (let y = 2 + T; y <= 25 + T; y += 0.25) { const c = ax(y), w = lw(y); for (let x = c - w; x <= c + w; x += 0.25) P(G, x, y, Math.abs(x - c) > w - 1.3 ? (x < c ? 'G' : 'Y') : 'k'); }
  for (let y = T - 2; y <= T + 3; y += 0.25) { const c = ax(y); for (let x = -2.8; x <= 2.8; x += 0.25) P(G, c + x, y, y > T + 0.5 && y < T + 1.75 ? (x < 0 ? 'G' : 'Y') : x < -1.6 ? 'f' : x < 0.4 ? 'm' : 'j'); }   // 口金（黒鉄の筒・金の帯・輪の頂を抱く）
  for (let y = 24 + T; y <= 27 + T; y++) { const c = ax(y); for (let x = c - 2.6; x <= c + 2.6; x += 0.5) P(G, x, y, x < c ? 'G' : 'Y'); }   // 輪の座
  { // 大鎌の刃：口金の左面 (ax(T)−2.6, T+0.5) から左へ 19・下へ 10 曲がる。半幅は根元 3.4 → 先 0.3。峰（上）は骨鉄の光 'f'・身は黒鉄（上 'm'・下 'j'）・刃（下＝曲がりの内側）は深紅 1.0・切っ先は 'A'
    //   落とした稿＝左 15・下 7・根元 3.0 → 等倍で小鎌に見えた
    const Ax = ax(T) - 2.6, Ay = T + 0.5;
    for (let u = 0; u <= 1; u += 0.002) {
      const cx = Ax - 19 * u, cy = Ay - 3 * u + 13 * u * u, dx = -19, dy = -3 + 26 * u, L = Math.hypot(dx, dy), nx = -dy / L, ny = dx / L;   // 法線＝峰の側（上）が正
      const w = 3.4 - 3.1 * Math.pow(u, 1.5);
      for (let s = -w; s <= w; s += 0.25) P(G, cx + nx * s, cy + ny * s, u > 0.95 ? 'A' : s < -w + 1.0 ? 'R' : s > w - 0.9 ? 'f' : s > 0 ? 'm' : 'j');
    }
  }
  const MX = ax(13.5 + T), MY = 13.5 + T, RX = 3.4, RY = 4.4, SH = 0.1;                                                // 輪の中の金の宝珠（滑らかな金の卵・顔は無い）
  for (let y = -RY; y <= RY; y += 0.25) for (let x = -RX; x <= RX; x += 0.25) {
  ...
  for (const [y, s] of [[15 + T, -1], [15 + T, 1], [18.5 + T, -1], [18.5 + T, 1], [22 + T, -1], [22 + T, 1]]) {         // 深紅の遊環（3対・輪の下半分の縁に掛かる）
    const c = ax(y) + s * (lw(y) + 0.6); DISC(G, c, y, 1.7, 'k'); DISC(G, c, y, 1.25, 'R'); P(G, c, y, 'k'); P(G, c - 0.5, y - 0.5, 'A');
  }
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝錫杖（獣を追い払う音の杖）に死神の鎌を継ぎ、梵鐘（鎮めの鐘）を金棒にした。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は刃を継いだ錫杖、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は尖った金の輪の頂から黒鉄の大鎌の刃（刃は深紅）を左へ出した錫杖＝殺さないための音の杖に死神の鎌を継いだ戒破りの武具。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は禍々しい月牙（深紅の縁・切っ先の光・後ろに影の月を従える）。後の左手は無く、右肩の上だけに翼の列が立つ＝左右非対称。冠の簾と帯の玉は紫＝最高位の色。'
