// 「蒼神骸華」（そうしんがいか）＝マキナ四神柱の頂点・すべてのマキナの創造主。第3案（2026-09-15 21:18 FB を受けて第2案に退廃を重ねた）。
//
// 第2案への FB（21:18）：「かなりいい。もっと退廃さを。他の3神と比較して荘厳さというより静謐さが強すぎる。静謐と退廃のギリギリのバランスを」
//   → 第3案＝**姿勢と光は静かなまま（静謐）、表面と縁を腐らせる（退廃）**。境界は体の中心線：左半分が腐り、右半分は清い。
//     ・腐蝕の色は錆（o/O・暖色）。第2案の暗い紫灰（w/u）は黒い衣に沈んで等倍で見えなかった＝蒼との補色で見せる
//     ・左肩から膝へ走る亀裂に神経光が点々と漏れ、亀裂の左に錆が滲む／落ちた左の肩当て（骨髄）／切れて垂れる瓔珞／帯の下の錆の垂れ／帯の金箔の剥がれ
//     ・蓮華座から左膝→左肩まで這い上がって枯れた錆の蔓と枯れ花／左下の衣と膝の黴／骨の面（襟・肩当て・左腕）の錆の斑点／左前腕の亀裂
//     ・円光＝外縁に宝珠7つ（左2つは抜けて穴）・欠けの周りは硝子が曇る・帯を横切る亀裂／舟形光背＝左の帯が曇り亀裂
//     ・蓮華座＝左端が崩れて瓦礫・枯れた蓮弁・落ちて転がる光背の欠片・散った枯れ花弁／胸の蓮華の左2弁は枯れる／冠の左の山は折れて骨髄
//     ・内側の空間＝左に落ちる枯れ花弁・右に種子から昇る光の粒（死と生を左右に振り分ける）
//
...
export const PAL = {
  ...
  d: '#2a1730',                                                           // 骨髄（折れ口）
  o: '#5a2e1e', O: '#9a5a30',                                             // 錆（亀裂から滲む・骨の斑点・枯れた蔓の茎）＝蒼との補色で退廃を等倍でも見せる
};
...
const MANDORLA = (() => {
  ...
  for (let y = 0; y < MAN_H; y++) {
    ...
    for (let x = 0; x < MAN_W; x++) {
      ...
      if (left && y >= 10 && y <= 46 && d < 2.0 && dither(x, y) && (y * 7 + x) % 3 === 0) ch = 'w';   // 折れの周りは硝子が曇る
      P(G, x, y, ch);
    }
  }
  ...
  // 退廃：帯の左に亀裂と神経光。内側＝左に落ちる枯れ花弁（u/w）・右に種子から昇る光の粒（N/C）＝死と生を左右に振り分ける
  for (const y of [33, 41]) { const hw = hwAt(y); LN(G, cx - hw + 0.5, y - 1, cx - hw + TH - 0.5, y + 1, 'k'); P(G, cx - hw + 2, y, 'c'); }
  for (const [wx, wy, ch] of [[-30, -6, 'u'], [-26, -1, 'w'], [-29, -13, 'u'], [-24, 4, 'w'], [-28, 8, 'u'], [-22, -19, 'u'], [-27, -24, 'w'],
    [24, 10, 'N'], [29, -1, 'C'], [22, 12, 'N'], [30, -12, 'N'], [31, -3, 'C']]) P(G, cx + wx, wy + 41, ch);
  OUTLINE(G);
  return R(G);
})();
...
// =====================================================================
const HALO_W = 37, HALO_H = 37;
const HALO = (() => {
  const G = g(HALO_W, HALO_H), c = 18;
  for (let y = 0; y < HALO_H; y++) for (let x = 0; x < HALO_W; x++) {
    ...
    if (r > 15.6) ch = lit > 6 ? 'G' : lit < -6 ? 'y' : 'Y';
      ...
      if (((ang > 112 && ang < 150) || (ang > 190 && ang < 238)) && dither(x, y)) ch = 'w';   // 欠けの周りは硝子が曇る（死んだ硝子）
    }
    P(G, x, y, ch);
  }
  for (const a of [150, 190]) for (let r = 11.2; r <= 17.2; r += 0.25) AT(G, c, c, r, r, a, r > 12 && r < 16 ? 'd' : 'k');   // 欠けの断面
  P(G, c - 13.5, c + 9, 'C');
  for (const a of [216, 234, 252, 270, 288, 306, 324]) {                                          // 外縁の宝珠7つ（左の2つは抜けて穴）
    const t = a * Math.PI / 180, sx = c + Math.cos(t) * 16.3, sy = c + Math.sin(t) * 16.3;
    if (a <= 234) { DISC(G, sx, sy, 1.3, 'k'); P(G, sx, sy, 'd'); }
    else { DISC(G, sx, sy, 1.4, 'G'); P(G, sx - 0.5, sy - 0.5, 'W'); P(G, sx + 0.7, sy + 0.7, 'Y'); }
  }
  for (const a of [225, 300]) {                                                                    // 帯を横切る亀裂と神経光
    const t = a * Math.PI / 180;
    LN(G, c + Math.cos(t) * 10, c + Math.sin(t) * 10, c + Math.cos(t) * 15.4, c + Math.sin(t) * 15.4, 'k');
    if (a === 225) P(G, c + Math.cos(t) * 12.5, c + Math.sin(t) * 12.5, 'c');
  }
  OUTLINE(G);
  return R(G);
})();
...
const PEDESTAL = (() => {
  ...
  const row = (y0, h, n, pitch, up, chip, wither = []) => {
    for (let i = 0; i < n; i++) {
      const px = cx + (i - (n - 1) / 2) * pitch, dead = wither.includes(i);
      for (let dy = 0; dy < h; dy += 0.25) {
        ...
        for (let k = -w; k <= w; k += 0.25) {
          ...
          if (dead) ch = ch === 's' ? 'u' : ch === 'j' ? 'k' : 'w';                        // 枯れた蓮弁
          P(G, px + k, y0 + dy, ch);
        }
      }
      if (chip !== i) P(G, px, up ? y0 : y0 + h - 1, dead ? 'u' : 'n');
    }
  ...
  row(0, 6, 6, 12.6, true, 0, [1]);
  for (let x = 2; x <= 75; x++) { const u = (x - 2) / 73; P(G, x, 6, u < 0.3 ? 'G' : u < 0.62 ? 'Y' : 'y'); P(G, x, 7, u < 0.3 ? 'Y' : 'y'); }
  row(8, 3, 7, 10.8, false, -1, [0, 1]);
  for (let x = 1; x <= 76; x++) P(G, x, 11, 'j');
  // 退廃：欠けた蓮弁の上に落ちて転がる光背の欠片（蒼硝子）・敷茄子に散った枯れ花弁・左端の敷茄子は錆びる
  P(G, 8, 3, 'N'); P(G, 7, 4, 'k'); P(G, 8, 4, 'P'); P(G, 9, 4, 'Q'); P(G, 10, 4, 'k'); P(G, 6, 5, 'k'); P(G, 7, 5, 'P'); P(G, 8, 5, 'Q'); P(G, 9, 5, 'q'); P(G, 10, 5, 'k');
  for (const [x, y, ch] of [[15, 6, 'u'], [24, 7, 'w'], [33, 6, 'u'], [19, 7, 'u'], [45, 7, 'w']]) P(G, x, y, ch);
  for (let x = 2; x <= 12; x++) for (let y = 6; y <= 7; y++) if (dither(x, y)) P(G, x, y, 'y');
  for (let y = 0; y <= 10; y++) for (let x = 0; x <= 6 - Math.floor(y / 4); x++) P(G, x, y, '.');    // 左端は崩れて無く、瓦礫が転がる
  for (const [x, y, ch] of [[1, 9, 'm'], [2, 9, 'f'], [3, 10, 'm'], [0, 10, 'j'], [4, 10, 'j'], [2, 10, 'm'], [5, 9, 'w'], [3, 8, 'u'], [1, 10, 'f'], [6, 10, 'o'], [4, 9, 'o']]) P(G, x, y, ch);
  for (let x = 7; x <= 14; x++) for (let y = 8; y <= 10; y++) { const ch = GET(G, x, y); if ((ch === 's' || ch === 'f' || ch === 'm') && (x * 5 + y * 3) % 4 === 0) P(G, x, y, 'o'); }   // 崩れた際の錆
  OUTLINE(G);
  return R(G);
})();
...
const TORSO = (() => {
  ...
  for (let d = 0; d <= 1; d += 0.25) ARC(G, cx, 2, 12 + d, 6.5 + d, 25, 128, d < 0.5 ? 'G' : 'Y');   // 瓔珞（左で切れている）
  for (const a of [60, 90]) { AT(G, cx, 2, 13.5, 8, a, 'G'); AT(G, cx, 2, 14.5, 9, a, 'W'); }
  LN(G, cx - 7.5, 7.5, cx - 9, 12, 'Y'); P(G, cx - 9, 13, 'G'); P(G, cx - 9, 14, 'W');               // 切れた鎖の端が垂れる
  ...
  // 退廃：左肩から膝へ走る亀裂と神経光・襟の亀裂・右の衣にも髪の毛ほどの亀裂・落ちた左の肩当て（上の層が無く骨髄）・
  //        帯の下の錆の垂れ・左下の衣と膝に黴と塵・蓮華座から這い上がって枯れた蔓と枯れ花・種子から昇る光の粒（右）・帯の玉飾り
  LN(G, cx - 15, 9, cx - 12, 16, 'o'); LN(G, cx - 12, 16, cx - 14, 22, 'o'); LN(G, cx - 14, 22, cx - 10, 28, 'o');   // 亀裂の左に滲む錆
  LN(G, cx - 14, 9, cx - 11, 16, 'k'); LN(G, cx - 11, 16, cx - 13, 22, 'k'); LN(G, cx - 13, 22, cx - 9, 28, 'k'); P(G, cx - 11, 16, 'c'); P(G, cx - 13, 22, 'c');
  LN(G, cx - 3, 4, cx - 2, 8, 'k'); P(G, cx - 3, 6, 'c'); P(G, cx - 4, 5, 'o'); P(G, cx - 3, 8, 'o');
  LN(G, cx + 9, 28, cx + 11, 34, 'k'); P(G, cx + 10, 30, 'o');
  for (let x = cx - 17; x <= cx - 10; x++) { P(G, x, 4, 'k'); P(G, x, 5, x > cx - 16 && x < cx - 11 ? 'd' : 'k'); P(G, x, 6, 'm'); }
  for (const [x, y, ch] of [[cx + 10, 27, 'O'], [cx + 10, 28, 'o'], [cx + 14, 27, 'O'], [cx + 14, 28, 'o'], [cx + 14, 29, 'o'], [cx + 3, 27, 'o'], [cx - 4, 27, 'O'], [cx - 4, 28, 'o'], [cx - 4, 29, 'o']]) P(G, x, y, ch);   // 帯の下の錆の垂れ
  for (let y = 27; y < TORSO_H; y++) for (let x = cx - 23; x <= cx - 9; x++) { const ch = GET(G, x, y); if (ch && ch !== '.' && ch !== 'k' && (x * 3 + y * 5) % 7 === 0) P(G, x, y, (x + y) % 2 ? 'o' : 'w'); }
  for (let y = 30; y < TORSO_H; y++) for (let x = cx + 9; x <= cx + 20; x++) { const ch = GET(G, x, y); if (ch && ch !== '.' && ch !== 'k' && (x * 3 + y * 5) % 13 === 0) P(G, x, y, 'o'); }
  const vine = (pts) => { for (let i = 0; i + 1 < pts.length; i++) LN(G, pts[i][0], pts[i][1], pts[i + 1][0], pts[i + 1][1], 'o'); for (const [x, y] of pts) P(G, x, y, 'O'); };
  const blossom = (x, y) => { P(G, x, y, 'u'); P(G, x - 1, y, 'w'); P(G, x + 1, y, 'w'); P(G, x, y - 1, 'w'); P(G, x, y + 1, 'w'); };
  vine([[cx - 22, 39], [cx - 20, 34], [cx - 16, 31], [cx - 12, 28], [cx - 10, 30]]);
  vine([[cx - 15, 39], [cx - 14, 36], [cx - 11, 34]]);
  blossom(cx - 20, 33); blossom(cx - 12, 28); blossom(cx - 11, 35);
  for (const x of [cx - 12, cx - 6, cx, cx + 6, cx + 12]) P(G, x, 25, 'W');
  P(G, cx + 12, 13, 'N'); P(G, cx + 14, 10, 'C');
  // さらに一段：蔓は左膝から左肩まで這い上がる・亀裂には神経光が点々と漏れる・帯の金箔が左で剥がれる・骨の面（襟の左・左の肩当て）は風化の斑点・右の肩当ての下に金の縁
  vine([[cx - 21, 30], [cx - 19, 24], [cx - 18, 18], [cx - 16, 12], [cx - 14, 8]]); blossom(cx - 19, 24); blossom(cx - 16, 12);
  P(G, cx - 12, 12, 'c'); P(G, cx - 10, 26, 'c');
  for (const [x, y] of [[cx - 15, 25], [cx - 16, 25], [cx - 9, 26], [cx - 17, 26], [cx - 20, 25], [cx - 21, 26]]) P(G, x, y, 'j');
  for (let y = 0; y <= 4; y++) for (let x = cx - 8; x <= cx - 2; x++) { const ch = GET(G, x, y); if ((ch === 's' || ch === 'f') && (x * 5 + y * 3) % 6 === 0) P(G, x, y, 'o'); }
  for (let y = 7; y <= 9; y++) for (let x = cx - 19; x <= cx - 12; x++) { const ch = GET(G, x, y); if ((ch === 's' || ch === 'f') && (x * 5 + y * 3) % 5 === 0) P(G, x, y, 'o'); }
  for (let x = cx + 12; x <= cx + 20; x++) P(G, x, 10, 'Y');
  for (let x = cx - 19; x <= cx - 15; x++) P(G, x, 10, 'y');
  OUTLINE(G);
  return R(G);
})();
...
const CROWN = (() => {
  ...
  for (let y = 4; y <= 5; y++) for (let x = 3; x <= 6; x++) P(G, x, y, '.');                          // 退廃：左の山は頂が折れて骨髄が覗く
  P(G, 3, 6, 'k'); P(G, 4, 6, 'd'); P(G, 5, 6, 'd'); P(G, 6, 6, 'k');
  for (let y = 9; y <= 11; y++) for (let x = 0; x <= 4; x++) if (dither(x, y)) P(G, x, y, y === 11 ? 'o' : 'y');      // 帯の左端は錆びる
  P(G, 6, 10, 'q'); P(G, 13, 10, 'q');                                                               // 帯の暗い宝石（明るくしない＝目にならない）
...
const LOTUS = (() => {
  ...
  for (let a = -90; a < 270; a += 45) {
    const t = a * Math.PI / 180;
    petal(G, c + Math.cos(t) * 1.8, c + Math.sin(t) * 1.8, c + Math.cos(t) * 6.4, c + Math.sin(t) * 6.4, 2.0, 0, { gilt: true, withered: a === 135 || a === 180 });   // 左の2弁は枯れる
  }
...
const ARM_L = (() => {   // 肩 (22,4) → 肘 (17,19) → 手首 (23,27) → 掌 (24,31)・指は下へ
  ...
  palm(G, 24, 31, 1, 1, 2);
  LN(G, 18.3, 24.9, 22.3, 21.9, 'k'); P(G, 20, 23, 'd'); P(G, 21, 23, 'd'); P(G, 19, 24, 'c');        // 退廃：前腕を横切る亀裂と骨髄・神経光
  for (const [x, y, ch] of [[21, 27, 'w'], [22, 28, 'u'], [26, 27, 'w'], [26, 28, 'u'], [27, 29, 'w']]) P(G, x, y, ch);   // 手首に絡む枯れ蔓
  for (let y = 4; y <= 18; y++) for (let x = 14; x <= 25; x++) { const ch = GET(G, x, y); if ((ch === 's' || ch === 'f') && (x * 5 + y * 3) % 7 === 0) P(G, x, y, 'o'); }   // 上腕の骨の錆の斑点
  P(G, 20, 24, 'O'); P(G, 19, 25, 'o');                                                                // 亀裂から滲む錆
  OUTLINE(G);
  return R(G);
})();
...
export const GAIKA = {
  ...
  concept: '蓮華座に結跏趺坐する創造主の亡骸。頭は無く、円光の中に金の冕冠だけが浮かび、冠の両端から蒼硝子の玉の簾が垂れる。'
    ...
    + '左に揃えた損傷：光背の一区間が折れ、円光の左下が欠け、左の簾が切れ、左肩に亀裂、蓮華の輪の左が欠けて肋が覗き、左膝は裂けて中が空、蓮華座の左端は崩れて瓦礫。'
    + '第3案の退廃：左半分が錆びる（亀裂から滲む錆・骨の斑点・帯の下の垂れ）。蓮華座から左肩まで枯れた蔓が這い上がり枯れ花が咲く。円光の宝珠が抜け硝子が曇る。'
    + '左に枯れ花弁が落ち、右に種子から光の粒が昇る。右半分は清いまま＝静謐と退廃の境界は体の中心線。',
  sprites: {
