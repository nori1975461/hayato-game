...
const ARML_W = 52, ARML_H = 152;
// ORIGIN 45 86
// 骸華の右手（画面左）。肩 (45,86)＝世界 (-17,-9)。肘 (31,101) → 拳 (10,92)＝世界 (-52,-3)。texture ＝ 世界 + (62, 95)
// 第20案：ユーザー指示「イメージと違う。もっとシンプルな棒に。だが荘厳さと退廃さを併せ持って。最強の闘神にふさわしい最凶の武具を」
//   部品を「一本の柄」と「両端の金の箍（たが）」だけに削る＝如意金箍棒（最強の闘神の棒の原型）の文法。玉・鈷・節・巻きの握りは全部捨てた。
//   柄＝幅 5 ドット・長さ 94 の黒鉄。垂直に立てる（第19案までの 6.8° の傾きは無地の棒だと縁が段々になる＝垂直なら線が澄み、右の斜めの梵鐘と対になる）。
//     紫の月に接しないよう拳を世界 −52 へ（第19案は −46）。頂は世界 −78・下端 +46。
//   箍＝両端とも同じ形・15 行：金の環 → 深紅の帯 → 鈍い金の筒 → 金の環 → 筒 → 金の環 → 蓋。筒を鈍く環を明るく＝「三重の箍」に読ませる。
//   荘厳と退廃は柄の 5 列の色だけで出す（形は増やさない）。既定 C＝左の縁が鈍い金（金覆輪＝鞍や鍔の縁を金で覆う飾りの発想）・芯に深紅の一筋（血の樋）・残りは黒。
//   比較用に柄の仕上げだけを変えた 4 本：A＝黒鉄の無地／B＝A＋深紅の一筋／C＝B＋金の縁／D＝血の色の漆（箍の帯は黒鉄）。
//   落とした稿＝傾けた太い棒（鉄パイプ）・銘の短冊（等倍で点線）・握りの上下の金の環（腕釧と重なり雑然）・太い深紅の芯（赤い棒になる）・金の両縁（リボン）・両端の鈷（槍になる）。
const ROD_DEFAULT = 'C';
const ROD_SPECS = { A: { cols: 'fmjjj' }, B: { cols: 'fjRjj' }, C: { cols: 'YjRjj' }, D: { cols: 'Rrrrk', ironBand: true } };   // cols＝柄の 5 列（左＝光の側 → 右）
export const GAIKA_ROD_LABELS = { A: 'A: BLACK IRON', B: 'B: A + CRIMSON VEIN', C: 'C: B + GOLD EDGE', D: 'D: BLOOD RED SHAFT' };
const armL = (rod) => {
  const sp = ROD_SPECS[rod] || ROD_SPECS[ROD_DEFAULT], G = g(ARML_W, ARML_H);
  const HW = 2.4, FX = 10, FY = 92, S0 = -33, S1 = 60;
  const S = (s, t, ch) => P(G, FX - t, FY - s, ch);                                   // (s,t)：s＝拳から上へ・t＝左（光の側）へ
  // --- 柄
  for (let s = S0; s <= S1 + 0.01; s += 0.2) for (let t = -HW; t <= HW + 0.01; t += 0.2) S(s, t, sp.cols[2 - Math.round(t)]);
  // --- 両端の箍（同じ形）。dir＝+1 上 / −1 下。行 qi：環 0-1／深紅の帯 2-3／筒 4-6／環 7-8／筒 9-11／環 12-13／蓋 14
  const cap = (s0, dir) => { for (let q = 0; q <= 14.4; q += 0.2) {
    const qi = Math.round(q), ring = qi <= 1 || qi === 7 || qi === 8 || qi === 12 || qi === 13, band = qi === 2 || qi === 3, lid = qi === 14;
    const w = ring ? HW + 2 : HW + 1, n = Math.round(w);
    const low = ring && (dir > 0 ? qi === 0 || qi === 7 || qi === 12 : qi === 1 || qi === 8 || qi === 13);   // 環の下の行は一段暗く＝数珠の丸み
    for (let t = -w; t <= w + 0.01; t += 0.2) { const k = Math.round(t); let ch;
      if (band) ch = sp.ironBand ? (k >= 1 ? 'm' : 'j') : k >= n ? 'A' : k >= 0 ? 'R' : 'r';
      else if (lid) ch = dir > 0 ? (k >= 1 ? 'G' : 'Y') : 'y';
      else if (ring) ch = low ? (k >= n - 1 ? 'G' : k >= -1 ? 'Y' : 'y') : (k >= n ? 'W' : k >= n - 2 ? 'G' : k >= -1 ? 'Y' : 'y');
      else ch = k >= n - 1 ? 'Y' : 'y';                                               // 筒は鈍い金
      S(s0 + dir * q, t, ch); }
  } };
  cap(S1 + 1, 1); cap(S0 - 1, -1);
  // --- 腕（棒の上に描く＝拳が柄を握る）
  arm(G, [[45, 86], [31, 101], [FX, 93]], 4.5, 3.4);
  armlet(G, [45, 86], [31, 101], 0.5, 4.6, true); armlet(G, [31, 101], [FX, 93], 0.72, 3.9);                                    // 臂釧（深紅の玉）と腕釧＝金
  fist(G, FX, 92, 4.3, 3.7);
  OUTLINE(G);
  return R(G);
};
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像と倒錯で語る：頭は無く円光の中は空。武具はどれも「鎮めの道具を武器に変えたもの」＝祀って鎮めるために立てられた御柱（みはしら）を引き抜いて棒にし、梵鐘（鎮めの鐘）を金棒にした。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は鎮めの御柱の棒、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は「鎮めの御柱（しずめのみはしら）」の棒＝神を数える字が「柱」であるとおり、祀って鎮めるために立てられた柱そのものを、祀られる側の闘神が引き抜いて棒にした。形は一本の柄と両端の金の箍（たが）だけ＝如意金箍棒（黒い神珍鉄の両端に金の箍・元は海の深さを測った重り）と同じ文法で、玉も刃も節も無い。黒鉄の柄は垂直に立ち、左の縁に鈍い金の一筋、芯に深紅の一筋（血の樋）。箍は鈍い金の筒に明るい金の環が三つ、根元に深紅の帯。飾りで語らず、長さと真っ直ぐさと三つの色（黒・鈍い金・深紅）で語る。'
...
function build(opts = {}) {   // opts.sword＝曲刀の中の左手（baseR）を付ける変種（第14案改の比較用・既定は無し）
  ...
  const rig = [
    ...
    { role: 'armL', tex: 'armL', ox: -17, oy: -9, origin: [45 / ARML_W, 86 / ARML_H] },
