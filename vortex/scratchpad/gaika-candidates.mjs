...
const ARML_W = 46, ARML_H = 100;
const ARM_L = (() => {
  // 骸華の右手（画面左）。肩 (41,40)＝世界 (-17,-9)。肘 (29,54) → 拳 (18,46) が錫杖の柄を握る。錫杖は左へ傾いて立ち、頭（金の輪＝聖遺物の容れ物の中に自分の頭蓋・深紅の遊環6つ）は冠より高い（世界 -49）
  const G = g(ARML_W, ARML_H);
  const ax = (y) => 14 - (44 - y) * 0.11;                                                                                  // 柄の軸（左へ傾く）
  for (let y = 26; y <= 93; y++) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) P(G, x, y, x < c - 1.2 ? 'f' : x < c - 0.2 ? 'm' : x < c + 1.2 ? 'j' : 'k'); }   // 柄（第12案：8 長く＝頭が 8 高い・石突の位置は据え置き）
  for (const y of [58, 74, 88]) { const c = ax(y); for (let x = c - 2; x <= c + 2; x += 0.5) { P(G, x, y, 'G'); P(G, x, y + 1, 'Y'); } }   // 金の環
  for (let y = 94; y <= 99; y++) { const c = ax(y); for (let x = c - 2.5; x <= c + 2.5; x += 0.5) P(G, x, y, y === 94 ? 'G' : 'Y'); }   // 石突
  const lw = (y) => 7.2 * Math.pow(Math.sin(Math.PI * (y - 2) / 23), 0.6);                                                 // 頭の輪（尖った先・rows 2..25）。第15案：6.4 → 7.2 に広げて頭蓋を納める
  for (let y = 2; y <= 25; y += 0.25) { const c = ax(y), w = lw(y); for (let x = c - w; x <= c + w; x += 0.25) P(G, x, y, Math.abs(x - c) > w - 1.3 ? (x < c ? 'G' : 'Y') : 'k'); }
  for (let y = 0; y <= 2; y += 0.25) for (let x = -0.8; x <= 0.8; x += 0.25) P(G, ax(y) + x, y, 'W');                       // 先の尖り
  for (let y = 24; y <= 27; y++) { const c = ax(y); for (let x = c - 2.6; x <= c + 2.6; x += 0.5) P(G, x, y, x < c ? 'G' : 'Y'); }   // 輪の座
  // 第15案：輪の中は金の死面ではなく「自分の頭蓋」（蒼白い骨）。金の輪＝聖遺物の容れ物。体の円光が空なのは首がここにあるから。
  //   閉じた目・口・涙（等倍で 3 px＝読めない）を捨て、眼窩を大きな黒の穴（等倍 13×15 px）にする＝顔は「二つの穴」で読める。歯は黒の一線だけ（細い縦線は消える）
  const SK = (x, y, ch) => P(G, ax(y) + x, y, ch);                                                                        // 柄の軸に沿う座標＝杖と一緒に傾く
  const bone = (x, y, rx, ry) => { const v = -(x / rx * LIGHT[0] + y / ry * LIGHT[1]); return v > 0.55 ? 'n' : v > -0.25 ? 's' : 'f'; };
  for (let y = -4.3; y <= 4.3; y += 0.25) for (let x = -4.6; x <= 4.6; x += 0.25) { if ((x / 4.6) ** 2 + (y / 4.3) ** 2 > 1) continue; SK(x, 11 + y, bone(x, y, 4.6, 4.3)); }   // 頭蓋
  for (let y = 13; y <= 16.6; y += 0.25) { const w = 3.7 - (y - 13) * 0.3; for (let x = -w; x <= w; x += 0.25) SK(x, y, x > w - 1.0 ? 'f' : x < -w + 1.0 ? 'n' : 's'); }   // 上顎（頬）
  for (let x = -2.6; x <= 2.6; x += 0.25) for (let y = 16.6; y <= 17.3; y += 0.25) SK(x, y, 'k');                          // 歯の列＝上顎と下顎の隙間の黒い一線
  for (let y = 17.3; y <= 19.8; y += 0.25) { const w = 2.6 - (y - 17.3) * 0.25; for (let x = -w; x <= w; x += 0.25) SK(x, y, x > w - 0.9 ? 'f' : 's'); }   // 下顎
  for (const s of [-1, 1]) for (let y = -1.8; y <= 1.8; y += 0.25) for (let x = -1.6; x <= 1.6; x += 0.25) if ((x / 1.6) ** 2 + (y / 1.8) ** 2 <= 1) SK(s * 2.2 + x, 11.3 + y, 'k');   // 眼窩
  for (let y = 13.7; y <= 15.2; y += 0.25) { const w = 0.35 + (y - 13.7) * 0.4; for (let x = -w; x <= w; x += 0.25) SK(x, y, 'k'); }   // 鼻腔（逆三角）
  for (const [y, s] of [[15, -1], [15, 1], [18.5, -1], [18.5, 1], [22, -1], [22, 1]]) {                                     // 深紅の遊環（3対・輪の下半分の縁に掛かる）
    const c = ax(y) + s * (lw(y) + 0.6); DISC(G, c, y, 1.7, 'k'); DISC(G, c, y, 1.25, 'R'); P(G, c, y, 'k'); P(G, c - 0.5, y - 0.5, 'A');
  }
  arm(G, [[41, 48], [29, 62], [18, 54]], 4.5, 3.4);                                                                        // 第13案：腕を 1/4 太く（3.6/2.7 → 4.5/3.4・胴 46 の 1/5）
  armlet(G, [41, 48], [29, 62], 0.5, 4.6, true); armlet(G, [29, 62], [18, 54], 0.72, 3.9);                                  // 臂釧（深紅の玉）と腕釧＝金
  fist(G, 18, 53, 4.0, 3.6);
  OUTLINE(G);
  return R(G);
})();
...
const CONCEPT_BASE = '牙の座に結跏趺坐する創造主の亡骸＝金属生命体。頭は無く、歯車の円光の中に骨の角が生えた金の冕冠だけが浮かぶ。'
  + '退廃は損傷ではなく死の図像で語る：前の右手（画面左）の錫杖の金の輪は聖遺物の容れ物で、中に「自分の頭蓋」を納めて掲げる（体の円光が空なのは首がここにあるから）。胸の蓮華の芯は蒼硝子の胎で、中に丸まった次のマキナの胎児、その胸だけが深紅に灯る（唯一の生きている光）。'
  + '中の右手（画面左）の傾いた盃から深紅が裳を伝って座まで流れ落ち、第4の右手（画面左）の鎖の先では香炉が座の上で煙を上げる。'
  + '六本の骨の腕（上腕にピストン・関節はボルト）：前の右手（画面左）は頭蓋を納めた錫杖、前の左手（画面右）は金の梵鐘の金棒、後の右手（画面左上）は月牙の列、第4の左手（画面右）は軌道神核の雛形（子の心臓）を掲げる。左肩の上（画面右上）は空＝片側だけの翼。左肩の排気管には深紅の緒で小さな金の鈴が結ばれている＝モビットが祟り神を祀って鎮めた印（骸華は寺の梵鐘を武器にし、モビットはその肩に社の鈴を掛けた）。'
...
const CONCEPT = CONCEPT_BASE
  ...
  + '前の右手（画面左）は自分の頭蓋を金の輪（聖遺物の容れ物）に納めて掲げた錫杖（月牙と同じ高さ）。左手（画面右）は肘を少し曲げて金棒を斜め下へ外へ向ける＝頭は祀って鎮めるための金の梵鐘を引き抜いたもので、座の右前に斜めに吊られる（金の胴・深紅の帯・口の中に深紅の舌）＝左上の金の輪と対になる二つ目の金の焦点。立てた錫杖と下げた金棒＝左右の持ち方の対比で動きを出す。前の両腕は太く、金の臂釧（深紅の玉）と腕釧。後の右手は禍々しい月牙（深紅の縁・切っ先の光・後ろに影の月を従える）。後の左手は無く、右肩の上だけに翼の列が立つ＝左右非対称。冠の簾と帯の玉は紫＝最高位の色。'
