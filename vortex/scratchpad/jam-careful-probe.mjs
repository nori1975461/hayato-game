...
// 2026-09-14 派生＝間合いを保つ走り方で被弾の下限を測る。使い方: node jam-careful-probe.mjs <seed> <portOffset> <mob|boss>   （env RUN_SEC 既定 470）
...
function pageInstall(cfg) {
  ...
  if (!window.__hitRec) {
    window.__hitRec = [];
    var r0 = window.__run, oh = r0.hitPlayer.bind(r0);
    // 2026-09-14 ⚠️最初の版は hitPlayer の呼び出しを全部数えていた（無敵0.55秒で弾かれた分も入って1252件・
    //   本体の jamSt.hits は81）。本体と同じ条件＝**jamSt.hits が増えた呼び出しだけ**を数える。
    r0.hitPlayer = function (d, x, y, c) {
      var n0 = r0.jamSt ? r0.jamSt.hits : -1;
      var mm = r0._moveMul == null ? 1 : r0._moveMul;
      var res = oh(d, x, y, c);
      if (r0.jamSt && r0.jamSt.hits > n0) {
        window.__hitRec.push({ mm: mm, c: c || '?', b: !!(r0.boss && r0.boss.active), t: Math.round(r0.elapsed) });
      }
      return res;
    };
  }
    ...
    try {
      ...
      if (cur) {
        if (active) {
          ...
          var stn = bs.state;
          if (stn !== prevState) { cur.states[stn] = (cur.states[stn] || 0) + 1; if (typeof stn === 'string' && /Tele$/.test(stn)) cur.teles++; prevState = stn; }
...
function pageDrive(profile) {
  ...
  // 2026-09-14「間合いを保つ」ボット：雑魚は 62→118px で早めに避け、ボスは 170→230px。
  //   被弾の下限を測るための走り方＝掴み間合い78pxの外へ出ない距離で投げ、密着しない。
  for (var i = 0; i < run.enemies.length; i++) { var e = run.enemies[i]; if (!e.active || e.stag) continue; push(e.x, e.y, e.isBoss ? 230 : 118, e.isBoss ? 3.6 : 2.2); }
  ...
  if (holding) {
    // 溜め中に密着しない。ボスへ向くのは「投げの向き＝移動の向き」を作るためだけなので、
    //   220px より遠いときだけ寄り、内側では押し返しに任せる（実装：狙いは移動方向と同じ）。
    if (profile === 'boss' && be) { var dB = Math.hypot(be.x - p.x, be.y - p.y); pull(be.x, be.y, dB > 220 ? 3.4 : 1.2); }
    else {
      var bx = 0, by = 0, n = 0;
  ...
  // 2026-09-14 ボスが 170px より近いときは溜めない（＝足を止めない）。溜め中は 74px/s・狙い中は 0 なので、
  //   近いのに溜めるのが接触被弾の本体。離れてから溜めて投げる＝「間合いを保つ」走り方の核。
  if (profile === 'boss' && be && Math.hypot(be.x - p.x, be.y - p.y) < 170) press = false;
...
async function main() {
  ...
  R.hitRec = await ev('JSON.stringify(window.__hitRec || [])');     // 2026-09-14 被弾のたびの「動けたか」
  ...
  if (R.hitRec) {
    const hr = JSON.parse(R.hitRec);
    const lock = hr.filter((h) => h.mm < 1).length, free = hr.length - lock;
    const byC = {}; for (const h of hr) byC[h.c] = (byC[h.c] || 0) + 1;
    console.log(`  被弾の内訳: 計${hr.length}（拘束中${lock}＝${Math.round(100 * lock / Math.max(1, hr.length))}% / 自由${free}）原因${JSON.stringify(byC)}`);
  }
