...
// 第1案 FB「顔と王冠がロボットに偏りすぎ」／第2案（金の仮面と円光）FB「完全に人の顔・四神柱と並べて違和感・体と合わない」
//   → 第3案＝第1案の体をそのまま使い、頭だけ「禍々しい鉄の巨人」に：
//   兜・バイザー・金冠（ロボットの記号）も顔（人の記号）も捨て、頭は鉄塊。頂は不揃いな鉄片の冠（1本折れ）、眉庇の影の下の窪みに
//   亀裂からシアンの光が漏れる「裂け目の眼」、顎は炉の格子の口で奥が白熱。右肩に鉄の棘。
//   四神柱の他の三体の「顔」は薔薇窓・冠面・眼＝どれも構造であって顔ではない。マオウも構造（亀裂と炉）で顔を作る
//
// 昇華の方針（作法を「鉄の巨人」の語彙で言い直す。識別＝人型・深紅×黒鋼・シアンの眼・深紅の炉心・右肩の砲）：
//   ①シルエットの格 … 鉄塊の頭（幅16・鉄片の冠）→ 耳まで上がる二層の肩当て（幅30・右は棘）→ 腰で締まる → 背後で裾が広がる外套（幅48）→ 最厚のブーツ
//   ②大きな面＋黒い溝＋面の中の明度差 … 胸板は3枚（縦の溝と溝の隣の光）、腹は蛇腹、肘・膝は段、拳は角で4本の指
//   ③素材ごとに4段 … 深紅 4段・黒鋼 4段・金 3段・シアン 2段・銅
//   ④黒で締める … OUTLINE
//   ⑤生きている一点 … 裂け目から漏れるシアンの光・炉の口の奥の白熱・U字ケージの炉心（深紅の心臓に白熱の芯）
//   ⑥退廃 … 鉄片の冠の折れ・外套の裾の裂け・腰から垂れる銅のケーブル・砲口の焦げ
//
...
import { g, P, DISC, RECT, OUTLINE, R, dither, validate } from './god-raster.mjs';
...
const BODY = (() => {
  ...
  // 頭＝鉄塊。頂は不揃いな鉄片の冠（金ではない・右の1本は折れて低い）、額は一枚岩の黒鋼、
  // 眉庇の影の下に窪みがあり、そこに裂け目の眼（core）が嵌る。窪みの底は炉の赤。顎は格子の口で、奥が白熱している
  for (const [x0, x1, top] of [[8, 8, 2], [10, 11, 0], [14, 15, 1], [18, 19, 2], [21, 21, 3]]) {          // 鉄片の冠
    for (let y = top; y <= 4; y++) for (let x = x0; x <= x1; x++) P(G, x, y, x === x0 ? 'h' : 'g');
  }
  P(G, 10, 0, 's'); P(G, 14, 1, 's');                                                                      // 高い2本の先だけ鈍い銀
  for (let x = 7; x <= 22; x++) P(G, x, 4, 'h');                                                           // 頂の稜線
  for (let y = 5; y <= 7; y++) for (let x = 7; x <= 22; x++) P(G, x, y, x < 11 ? 'h' : x < 17 ? 'g' : 'f');   // 額（一枚岩）
  for (let y = 4; y <= 7; y++) P(G, 15, y, 'k');                                                           // 額の溝
  P(G, 9, 6, 'j'); P(G, 20, 6, 'j');                                                                       // 鋲
  for (let x = 7; x <= 22; x++) P(G, x, 8, 'k');                                                           // 眉庇の影
  for (let y = 9; y <= 12; y++) for (let x = 8; x <= 21; x++) P(G, x, y, 'j');                             // 顔の窪み（core が嵌る）
  for (let x = 10; x <= 19; x++) if (dither(x, 12)) P(G, x, 12, 'd');                                      // 窪みの底に炉の赤
  for (let x = 9; x <= 20; x++) P(G, x, 13, 'h');                                                          // 顎の稜線
  for (let y = 14; y <= 15; y++) for (let x = 9; x <= 20; x++) P(G, x, y, y === 14 ? 'd' : 'm');           // 炉の口
  for (const x of [10, 13, 16, 19]) { P(G, x, 14, 'k'); P(G, x, 15, 'k'); }                                // 口の格子
  P(G, 15, 15, 'R'); P(G, 14, 15, 'R');                                                                    // 格子の奥の白熱
  for (let x = 9; x <= 20; x++) P(G, x, 16, 'k'); for (let x = 12; x <= 17; x++) P(G, x, 16, 'j');           // 顎の下と首
  P(G, 11, 16, 'o'); P(G, 18, 16, 'o');                                                                    // 首のケーブル
...
// ③ 裂け目の眼（core）14×3。整ったレンズではなく、鉄塊に走った亀裂からシアンの光が漏れている（両端と中央がぎざつく・中央が白熱）
const VISOR = (() => {
  const G = g(14, 3);
  for (let x = 0; x <= 13; x++) P(G, x, 1, 'c');
  for (const x of [2, 3, 9, 10]) P(G, x, 0, 'c'); for (const x of [0, 1, 12, 13]) P(G, x, 2, 'c');
  for (let x = 4; x <= 9; x++) P(G, x, 1, 'C'); P(G, 6, 1, 'w'); P(G, 7, 1, 'w'); P(G, 6, 0, 'C'); P(G, 7, 2, 'C');
  return R(G);
})();
...
const PAULDRON = (() => {                                                                                  // 右肩は鉄の棘を3本（禍々しさは肩に出す）
  const G = g(16, 13);
  for (const x of [2, 6, 10]) { P(G, x, 1, 'g'); P(G, x + 1, 1, 'g'); P(G, x, 0, 'h'); }
  shoulder(G, 2); OUTLINE(G); return R(G);
})();
const CELLPOD = (() => {
  const G = g(16, 14);
...
export const MAOU1 = {
  id: 'maou1',
  name: 'マオウレクス',
  concept: '黒鉄の板鎧の外套を翻す禍々しい鉄の巨人。頭は鉄塊で、頂は不揃いな鉄片の冠、眉庇の影の下の亀裂からシアンの光が漏れ、'
    + '顎は炉の格子の口で奥が白熱している。耳まで上がる二層の肩当て（右肩は鉄の棘）、右肩のレーザー砲、幅いっぱいの角拳。'
    + '胸板は3枚に割れ、U字の金のケージの奥で深紅の炉心が白熱する。'
