...
//   node render-gaika2-beforeafter.mjs [export 名=GAIKA2] [倍率=2.2] [中心の世界y=8] → gaika2-beforeafter.png
...
const name = process.argv[2] || 'GAIKA2', S = Number(process.argv[3]) || 2.2, wy = Number(process.argv[4] ?? 8), wx = Number(process.argv[5] ?? 0), file = process.argv[6] || './gaika2-beforeafter.png';
const out = makeCanvas(640, 360); rect(out, 0, 0, 640, 360, [40, 42, 64]);
[[OLD[name], 'BEFORE'], [NEW[name], 'AFTER']].forEach(([d, label], i) => {
