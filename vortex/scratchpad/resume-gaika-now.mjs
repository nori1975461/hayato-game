...
//   使い方: node vortex/scratchpad/resume-gaika-now.mjs            → 状態の表示＋ 8 支柱＋蝕刃（いまの頭と肩）と 12 肩当てをすっきり＋ブレードアンテナ の全身を横並びで gaika2-now.png（640×352）へ
//           node vortex/scratchpad/resume-gaika-now.mjs 3 4        → 番号で選んだ候補（1〜16・最大2つ）を gaika2-now.png へ
...
console.log('\n=== 4. 結論待ち＝①朔の座の絵（2〜4・ユーザー検討中）②蝕刃は⭐採用・支柱は指摘なし＝8 で進行 ③肩当て（10）と頭（11〜16）＝FB 待ち ===');
for (const [k, c] of Object.entries(CANDS)) console.log('  ' + k + '＝' + c.jp + '  ' + JSON.stringify(c.o));

const pick = process.argv.slice(2).map(Number).filter((n) => CANDS[n]).slice(0, 2), ids = pick.length ? pick : [8, 12];
