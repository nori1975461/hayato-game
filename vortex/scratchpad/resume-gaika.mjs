...
console.log('  胴の版: GAIKA2＝ザク型のひねり（第45稿からの既定＝style zaku2・route tuck・鋼。胸の下の角を落とし、その陰から管が出る）/ 比較用＝GAIKA2_NZ（ノイエ・ジール版・第44稿までの既定）/ GAIKA2_DOM / GAIKA2_ZAKU（第44稿のザク版＝短い管）/ GAIKA2_ZEONG');
...
console.log('\n=== 3. 不変と制約の検査 ===');
for (const f of ['check-gaika1-hash.mjs', 'check-gaika2-hash.mjs', 'check-gaika2-fanroot.mjs', 'check-gaika2-holes.mjs', 'check-gaika2-handskirt.mjs', 'check-gaika2-torso-width.mjs']) { try { console.log('  ' + execFileSync('node', [f], { cwd: here, encoding: 'utf8' }).trim().split('\n').pop()); } catch (e) { console.log('  (失敗) ' + f + ' ' + e.message.split('\n')[0]); } }
