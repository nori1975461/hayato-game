  ...
  boss: {
    ...
    tiers: [
      ...
      //    radius/glow/spawnDist も追随縮小（通常ボスの radius 64〜76 帯へ）。
      {
        ...
        // ⚠️ 尺の設計。初出は「20〜25秒（短く強烈）」で 75000＝実測21.6〜22.9秒に合わせたが、
        //    R36W2 実プレイFB「軌道神核との戦闘をもう少し長く楽しみたい。最適な時間はまかせる」。
        //    **35秒前後**に置く。根拠：①専用BGM（178BPM・16小節＝1周21.6秒）が1.6周鳴る＝曲の
        //    4段構成（名乗り→追い立て→主題→頂点）を最後まで聞かせられる ②攻撃3種が各3〜4回
        //    ずつ回る＝「読んで避ける」が2周目で上達として実感できる ③「もう少し」なので2倍には
        //    しない（40秒超は3本ゲージでも中だるみする）。
        //    HPは秒数ではなく実測DPSから引く（同じ設定でも±10%ぶれるため）。115000 の実測は
        //    39.6秒＝上端すぎた。殻閉じが3回とも成立して無敵4.2秒が乗り、有効DPSは 3249/秒
        //    （22秒時代の3550より低い＝殻の無敵中に投げが無駄撃ちされるぶん）。
        //    35 = hp/3249 + 4.2 → **100000**。
        // ⚠️ 攻撃は3種のまま。種類を足すより、同じ3種が2巡することで「読める」を作る。
        trueForm: {
          ...
          hp: 100000, radius: 92, spriteScale: 7.4, glowScale: 10.6,
          glowOuter: '#c98cff', glowInner: '#ffedb0',
          chaseSpeed: 110, bodyDamage: 38, gaugeSegments: 3,
          bulletTint: '#ffedb0',
