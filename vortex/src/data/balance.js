...
export const BALANCE = {
  ...
  hero: {
    ...
    billiard: {
      ...
      pitch: {
        windSweepDeg: 150,        // 振りかぶりで腕を後ろへ引く角度（溜め切りで最大）
        // ⚠️ 0.35 だと軽い溜めのとき 52〜81° しか引かず、ボールが体の**前**に残っていた（検証A2でNG）。
        //    90°を超えて初めて「後ろに引いた」に見えるので、下限を 0.68（＝102°）にした。
        windEaseBase: 0.68,
        // ⚠️ 18px は主人公の体半幅（12ドット×3.0÷2＝18px）とちょうど同じで、ボールが体に埋まっていた。
        //    「おもいっきり振りかぶる」には体の輪郭の外へ出す必要がある。
        windReach: 26, windReachMax: 40,
        motionSec: 0.17,          // 振り〜フォロースルーの全長
        releaseAt: 0.34,          // ★この割合の時点で手からボールが離れる（0.17×0.34＝約0.06秒）
        releaseReach: 34,         // ★離す瞬間の手の位置＝弾が生まれる場所。体の中心ではない
        followDeg: 34,            // 振り抜いて前へ行き過ぎる角度（フォロースルー）
        ghosts: 5, ghostSpanDeg: 52,   // 腕の残像（「高速スピードで腕を振って」を目に見せる担当）
        bodyLift: 8,              // 振りかぶりで体が持ち上がる高さ＝足を高く上げる（体高54pxに対して約15%）
        bodyLean: 0.22,           // 振りかぶりの反り(rad)
        bodyLungeLean: 0.26,      // 振り抜きの前傾(rad)
        bodyLunge: 6,             // 振り抜きで前へ踏み込む距離
        squash: 0.10,             // 踏み込んだ瞬間の潰れ（縦を縮めて横を広げる）
        moveMulWhileThrow: 0.25,  // 投げている0.17秒は踏ん張る。0にはしない（足止めのFBがあったため）
        dust: 10,                 // 踏み込みの土煙の粒数
      },
