...
export const BALANCE = {
  ...
  hero: {
    ...
    billiard: {
      ...
      // ★2026-09-21 掴み中に十字キーで狙いを 22.5°（16方向の1刻み）ずつ回すときの、1刻みの間隔。
      //   押しっぱなしならこの間隔で次の刻みへ進み、押した8方向に届いたら止まる。
      aimStepSec: 0.085,
      // 押しっぱなしで次の刻みへ進み始めるまでの待ち（初回だけ長い＝ふつうのキーリピートと同じ）。
      //   短すぎると軽く叩いただけで2刻み回り、長すぎると回し続けるのが遅くなる。
      aimRepeatSec: 0.22,
      ...
      shards: {
        ...
        bossHpCap: 0.05,
        // ★★R65 剥がれる条件を「予告を割ったとき」だけにしない。実プレイFB（息子さん）
        //   「装甲片がいつ出てくるかわからない」＝予告は**避けろ**の信号としてしか読めておらず、
        //   「いま叩けば装甲が剥がれる」という二つ目の意味が伝わっていなかった。結果、ボス戦の
        //   与ダメの3〜5割を担うはずの装甲片が実プレイで**一度も使われず**、ウズバルカン1体に
        //   232秒（ボットの9.3倍）かかっていた。
        //   ボスを最大HPの 20% ぶん削るたびに剥がれる＝普通に戦えば必ず出る（80/60/40/20%の
        //   4回×2枚＝8枚）。1枚は上限5%なので、全部当てても最大40%＝倒しきる量にはならない。
        //   予告割りの経路は不変＝割れば追加で剥がれ、よろけ中は×2.4。上手い遊びの得は残す。
        everyHpRatio: 0.2,
  ...
  hitFeel: {
    ...
    // ★R58 「止める予算」（実プレイFB「敵が多く出てきた際に、画面がゆっくりストップモーションの
    //   ようになってしまった」）。実測（scratchpad/r58-slowmo-diag.mjs）：敵130体でも 60fps・
    //   update 0.78ms で**重くはない**。止まって見えた正体は演出＝freezeT（ヒットストップ）と
    //   slowT（スローモーション）が敵の数に比例して連続発火し、**画面時間の50.4%が止まるか遅い**
    //   状態だった（通常の敵11体では22.2%）。1発ごとの上限（freezeCapSec）では防げない＝
    //   1発は短くても、毎フレーム誰かが立て直すので切れ目が無くなる。
    //   → 時間を止める権利を「直近 windowSec 秒のうち budget（割合）まで」の予算にする。予算を
    //     使い切っている間は、新しい停止/減速の**延長ぶん**を minFrac まで縮める（0にはしない＝
    //     手応えは残す・切れ目だけ作る）。
    //   値は模擬で掃引（test-core R58）：窓3秒／30%／下限25% → 静かな場面は -0.08pt で不変、
    //   混雑（模擬74%）は 31.5% へ。窓1秒だと静かな場面の山まで削るので窓は長く取る。
    //   minGrantSec：縮めた結果が2フレーム未満になる停止は見送る（1〜2フレームの停止は「止まった」
    //   でなく「引っかかった」に見え、混雑で量産されるとそれ自体がコマ落ちになる）。
    //   ★R59 crowd：予算を入れた後も「3体目・4体目のボスで敵が多すぎるとストップモーション」。
    //   実測（scratchpad/r58b-boss-crowd-diag.mjs・ボス戦＋敵94〜133体）：止まる割合は 27〜31% で
    //   予算どおりなのに、**止まる区間が 1秒に 3.1〜3.5 回×4〜5フレーム**（通常 2.4 回/秒）。
    //   同じ30%でも「0.3秒ごとに4フレーム止まる」がコマ送りの正体＝時間の合計では回数を縛れない。
    //   → 敵が lo 体以下なら従来どおり（間隔0）。hi 体以上なら、次の停止を始めるまで gapSec 空ける。
    //     bigSec 以上の要求（被弾 0.16・一気消し）は待たずに通す＝大事な手応えは消さない。
    //   狙い：混雑で 3.5 回/秒 → 1.5〜1.8 回/秒（1回の長さは変えない＝「たまにしっかり止まる」へ）。
    //   gapSec=1.0 の見込み：小さな停止は ≤1回/秒、被弾（ボットで 0.77回/秒）は素通り → 合計 ≤1.8回/秒。
    //   実測（同一プロセス対照・R59適用後）：ボス3戦 3.49→1.39 回/秒・ボス4戦 3.12→1.43 回/秒・
    //   1回の長さ 4〜5f→9〜11f・止まる割合 27〜30% は不変・通常場面 2.39→2.29 回/秒（22.4%）＝不変。
    timeStop: { windowSec: 3.0, budget: 0.30, minFrac: 0.25, minGrantSec: 0.034,
                crowd: { lo: 40, hi: 120, gapSec: 1.0, bigSec: 0.12 } },
  ...
  orbit: {
    ...
    // ★R62 ユーザー指示「ラゴンの対ボス倍率を0.3＝1突き4〜5にして」。SR の見せ場（R61でボスを狙うように
    //   した直後、0.12 では1突き2＝ボスHPの0.1%で「届くが微小」だった）。baseDamage 14 × 0.3 = 4.2 → 4、
    //   合成・段位で 15〜17 なら 5。ボス戦の主役は主人公（装甲片×2.5・とどめは主人公のみ）のまま。
    lagonBossMul: 0.30,
  ...
  archetypes: {
    ...
    // ★R46 trueFormRefill: 転生して軌道神核になった瞬間に足す発数。
    //   転生は同じ boss オブジェクトを使い回すので在庫が作り直されず、マオウレクス戦の
    //   2発を転生前に使い切ると**いちばん切り札が欲しい軌道神核戦で0発**になっていた。
    // ★R49 実プレイFB「軌道神核戦で特殊弾を＋2発にして」。転生時の補充を 1 → 3 発へ。
    //   R46 で「転生しても在庫が作り直されず0発になる」バグを塞いだときは1発だけ足したが、
    //   軌道神核は本編でいちばん長い戦闘（実測112.6秒）なので、refillSec=16 を挟んでも
    //   3発とも使い切れる。ここは切り札を出し惜しみする場所ではない。
    // ★R50追記: ビリッコ不在ランへの保証弾も一度実装したが、ユーザー判断で撤回
    //   （2026-09-02「ビリッコ引けなかったランでは特殊弾は不要。ビリッコの存在価値が薄れる」）。
    //   特殊弾はビリッコの専売のまま＝引き当てる価値を守る。
    AMMO: { firstDelaySec: 4, refillSec: 16, perBoss: 1, perFinal: 2,
            trueFormRefill: 3, trueFormDelaySec: 5 },
    // ★R45 実プレイFB「新たなモビットを創造して ①主人公を守る防御壁（命の盾・ボス戦ごとに
    //   1回のみ）②一時的に移動速度を1.5倍に上げる薬（爆速ドリンク・ボス戦ごとに1回のみ）
    //   ③ずっとなにもせずに欠伸ばかりして役に立たないが、軌道神核との戦闘に入ると覚醒し、
    //   命の盾・爆速ドリンク・体力回復をランダムに行う（このモビットの時だけ使用上限なし）」。
    //
    // 3つとも敵に一切触れない非戦闘アーキタイプ（HEAL/AMMO と同じ系譜）＝
    // 「仲間はとどめを刺せない」というビリヤード攻撃の設計の関門とは無関係に成立する。
    //
    // ---- ①命の盾（マモリン）----
    // ⚠️ 「守る」は**ピンチで出る**から意味がある。ボス戦の開始と同時に張ると、いちばん安全な
    //    時間帯を無敵で潰して終わる＝盾が1回も仕事をしない。HPが落ちた瞬間を引き金にする。
    // ⚠️ 引き金を厳しく（例30%）すると**一度も発動しないまま終わる**＝「入れていないのと同じ」。
    //    65% は R44 の実測（真の姿の戦闘で主人公は繰り返し半分以下まで落ちる）から選んだ。
    SHIELD: { hpTrigger: 0.65, durSec: 6, perBoss: 1, perFinal: 1 },
    // ---- ②爆速ドリンク（ドリンゴ）----
    // 指定どおり移動1.5倍。⚠️ 攻撃力ではなく**足だけ**が速くなる＝火力過多にはならず、
    //    「避けきれなかった攻撃が避けられる」＝②被弾の緊張感の側を動かす薬になる。
    // 注入はボス戦開始から8秒後（登場カットシーンと名乗りが終わって、実際に攻撃が始まる頃）。
    SPEED: { moveMul: 1.5, durSec: 10, delaySec: 8, perBoss: 1, perFinal: 1 },
    // ---- ③ネムッコ（欠伸）----
    // ★このモビットは**軌道神核（真マオウレクス第4形態）に入るまで一切働かない**。
    //   FB「軌道神核との闘い以外では明らかに役に立っていないことをプレーヤーがわかるように」＝
    //   何もしないだけでは「壊れている」と読まれるので、寝ている姿を積極的に見せる
    ...
    // ★R47 実プレイFB「引き当て超レア。一回り身体が大きく筋肉もりもりの武闘派。長い槍で
    //   勝手に敵を倒しに行く。このモビットだけ単独行動する。気絶させて弾にするのではない、
    //   完全に倒す（消滅させる）。しばらく戦ったら疲れをいやすために主人公のもとに帰ってきて
    //   肩で息をする。しばらくしたらまた戦いにいく。体力ゲージは不要」＝ラゴン。
    //
    // ⚠️ この子だけが「とどめを刺せる仲間」なので、置き方を間違えると R21W2 で潰した
    //    **最悪の構造がそのまま戻る**：仲間が画面外まで敵を掃除し、主人公に届く前に敵が消え、
    //    殴る機会（①）と殴られる脅威（②）が同時に失われる。3つの数値で縛ってある：
    //      ①huntRange 150 …主人公から離れられる上限。ビュー640×360の**必ず画面内**に収まる
    //        （allyMaxReach 132 より少しだけ外＝公転48の3倍離れる＝単独行動が絵で分かる）。
    //      ②huntSec 9 / pantSec 4.5 …狩っている時間は全体の約2/3。**休んでいる間は
    //        プレイヤーの狩り場が戻る**＝FBの「肩で息をする」がそのまま安全弁になっている。
    //      ③thrustSec 0.62 …1突き1体。範囲攻撃にしない（掃除機になると②が消える）。
    // ⚠️ よろけ（青白い輪＝主人公の獲物）には手を出さない。これは dealDamage の
    //    「よろけ中は manual 以外通らない」で担保される＝ラゴンも例外にしない。
    // ★出撃と帰りで速さを変える。sally（前線へ駆ける）は主人公より速く、return（帰る）は
    //   主人公より遅い＝**疲れて戻ってくる**が速度だけで読める。同じ値にすると、実測で
    //   帰りが 90秒中 1.6秒しか映らず「帰ってくる」がプレイヤーの目に入らなかった。
    // ★minStandoff＝狩っている間、主人公にこれより近づかない下限。
    //   ⚠️ これが無いと、敵は主人公へ向かって集まる性質のせいでラゴンも一緒に引き寄せられ、
    //      狩っている間の平均が 74.6px（公転48pxの1.55倍）まで落ちた＝実測。
    //   70〜150px の帯で戦う＝**前線を守る**役になり、主人公の周り（70px未満）は
    //   プレイヤーの狩り場として丸ごと残る（槍の間合い44pxでも 70-44=26px には届かない）。
    LANCER: { huntRange: 150, minStandoff: 70, moveSpeed: 168, sallySpeed: 232, returnSpeed: 110,
              huntSec: 9, pantSec: 4.5, homeRadius: 40,
              // ★reach は「見た目の槍の長さ」と一致させてある（描画は reach×2.0＝88px、
              //   ラゴンの体は 53px なので体の1.7倍＝FBの「長い槍」が絵でも成立する）。
              //   ⚠️ 見た目だけ伸ばして間合いを短いままにすると「刺さっているのに当たらない」
              //      という最悪の嘘になる（[[feedback_one_hit_one_circle]]）。
              reach: 44, thrustSec: 0.62, spriteScale: 3.3,
              // 刃の伸び縮みにかかる秒数（狩りへ出るとき点火・帰るときしまう）
              bladeSec: 0.35 },
  },
  ...
  capture: { dropRate: 0.25, eliteDropRate: 1.0, coreLifeSec: 10, fullPartyCoins: 50,
             // ★R60 R／SR のコアの寿命。エリートは投げで遠くで倒れることが多く、10秒だと取りに行く前に消える
             //   （実測 seed=999：ドリンゴのコアが拾われず消えた）。珍しいコアは1ラン3個しか無いので長く残す。
             rareCoreLifeSec: 22,
             // ★R61 最後のエリート（elite.times の末尾＝290秒）は SR 未所持なら SR コア（ラゴン／オーラジェリー）。
             //   R60後も SR は合成のみで1ラン約17%＝ラゴンが出ないまま。終盤に1回だけ確定の道を足す。
             lastEliteSR: true,
             ammoCoreSec: 38,
             // ★R45 取り逃したコアを置き直すまでの待ち（すぐ足元へ再出現すると
             //   「拾えなかった」という結果が消えて、ありがたみも緊張も無くなる）。
             ammoRetrySec: 12 },
  ...
  boss: {
    ...
    //   極小の小型ロボを大量に排出／突然の加速）」。マオウレクス（final）には一切触っていない。
    //
    // 【弾速の方針】遅い弾ほど大きく上げる（+30%）。既に速い machinegun/missile は +10% に留める。
    //   ⚠️ 公平性の算定（主人公は speed 148px/s・半径7）。いちばん速い machinegun 290px/s でも、
    //      交戦距離200px なら到達まで 0.69秒＝その間に横へ102px 動ける。当たるのに必要な
    //      ずれは 主人公7＋弾芯4＝11px なので、**見てから横に走れば必ず外れる**。
    //      至近60pxでも 0.21秒＝31px ＞ 11px。速くしてもダメージは1点も上げていない
    //      （緊張感は被弾量ではなく「避けた回数」で作る）。
    //   ⚠️ lifeSec は弾速に合わせて縮める＝射程（速度×寿命）を据え置く。縮めないと画面外まで
    //      延々飛ぶ弾が増えて、避けた後も後ろから戻ってくる（＝読めない盤面になる）。
    tiers: [
      // 1. 小ボス「コロガンナー」（~60秒）。マシンガン連射＋突進。phase2なし・撃破でプレイ続行。
      {
        ...
        // R52b: 264→290（+10%）。もともと最速級なので上げ幅は小さく、射程は lifeSec で据え置く
        //   （264×1.6＝422px → 290×1.5＝435px）。
        machinegun: { telegraphSec: 0.5, burstSec: 0.9, shotInterval: 0.08, bulletSpeed: 290,
                      bulletRadius: 3, damage: 6, spreadDeg: 14, lifeSec: 1.5 },
        // ★R52b 署名攻撃②「ローリングラッシュ」＝突然の加速（ユーザー案「突然ボスが加速してきたり」）。
        //   コロガンナー＝球体なので「転がって突っ込む」。dash（1回・長い・遅い）に対して
        //   **速く・短く・3回**で、1回ごとに向きを取り直す＝ジグザグに追ってくる。
        //   ⚠️ 公平性：1回の突進は 420×0.30＝126px 進み、向き直しの gapSec 0.16秒を挟む。
        ...
        ring: { telegraphSec: 0.5, count: 5, count2: 7, bulletSpeed: 156,
                bulletRadius: 4, damage: 12, lifeSec: 2.4 },
        summon: { count: 4, enemyId: 'chibit', ringRadius: 50 },
        idleSec: { afterSpawn: 2.5, betweenAttacks: [2.2, 2.2, 2.4, 2.2] },
      ...
      // 2. 小+ボス「ジェットバイパー」（~120秒）。円鋸カッター（ブーメラン）＋突進。
      {
        ...
        // R52b: 180→230（+28%）。ブーメランなので lifeSec は据え置く（戻りの尺が命）。
        cutter: { telegraphSec: 0.6, count: 2, speed: 230, spreadDeg: 40, bladeRadius: 9,
                  damage: 20, spinSpeed: 12, lifeSec: 2.4, returns: true },
        // ★R52b 署名攻撃②「ペンシルレーザー」＝細いレーザー砲（ユーザー案「細いレーザー砲にする」）。
        //   ウェイブロードの波動砲（極太・薙ぐ）との対比＝**細く・速く・複数本・薙がない**。
        //   予告0.4秒のあいだ機首から細い照準線が3本のびる。線は予告の**開始時点で固定**なので、
        //   「線が見えたら軸をずらす」がそのまま正解になる（追尾し続ける予告は読めない＝R43の教訓）。
        //   ⚠️ 公平性：予告0.4秒で主人公は59px 動ける。ビーム半幅3＋主人公7＝10px なので走れば外れる。
        //      3本は40°の扇（隣接20°）＝距離200pxで隣の線まで70px 空く＝逃げた先が別の線、にならない。
        pinlaser: { telegraphSec: 0.4, count: 3, spreadDeg: 40, beamWidth: 6, beamLength: 420,
                    damage: 18, activeSec: 0.18 },
        ...
        // R52b: 132→172（+30%）。射程据え置き（132×3.2＝422px → 172×2.5＝430px）。
        ring: { telegraphSec: 0.5, count: 7, count2: 9, bulletSpeed: 172,
                bulletRadius: 4, damage: 14, lifeSec: 2.5 },
        summon: { count: 5, enemyId: 'chibit', ringRadius: 55 },
        idleSec: { afterSpawn: 2.5, betweenAttacks: [2.0, 2.0, 2.2, 2.0] },
      ...
      // 3. 中ボス「ウズバルカン」（~180秒）。バルカン掃射＋アームスラム＋phase2「ぶちギレ」。
      {
        ...
        // R52b: 138→180（+30%）。「速射」の武器なのに主人公(148)と並んで歩けていた。
        //   射程据え置き（138×3.2＝442px → 180×2.5＝450px）。
        vulcan: { telegraphSec: 0.5, bursts: 3, perBurst: 9, sweepDeg: 14, bulletSpeed: 180,
                  bulletRadius: 4, damage: 15, lifeSec: 2.5 },
        // ★R52b 署名攻撃②「ドリルシェル」＝尖った大きめの弾（ユーザー案「弾のエフェクトを
        //   尖った大きめの弾にする」）。回転するドリル型の大弾を3発、時間差で撃つ。
        //   バルカン（小粒を大量に薙ぐ）との対比＝**大粒・少数・1発が重い**。
        //   弾は bendAtSec で**1回だけ**主人公の方へ緩く曲がる＝「まっすぐ避けて終わり」にしない。
        //   ⚠️ 公平性：曲がるのは1回・最大 bendDeg 46°まで。曲がった後は直進なので、
        //      曲がり切ったのを見てからもう一度横へ走れば必ず外れる（追い続ける弾にはしない）。
        //      判定半径9は見た目（32×20px＝r×3.6/2.2）と±20%以内で一致させてある。
        drill: { telegraphSec: 0.6, count: 3, shotInterval: 0.18, speed: 210, spreadDeg: 16,
                 radius: 9, damage: 22, spinSpeed: 16, bendAtSec: 0.38, bendDeg: 46, lifeSec: 2.6 },
        ...
        // R52b: 132→172（+30%）。射程据え置き（132×3.5＝462px → 172×2.7＝464px）。
        ring: { telegraphSec: 0.5, count: 7, count2: 11, bulletSpeed: 172,
                bulletRadius: 4, damage: 15, lifeSec: 2.7 },
        summon: { count: 6, enemyId: 'chibit', ringRadius: 60 },
        idleSec: { afterSpawn: 3, betweenAttacks: [2.5, 2.4, 2.6, 2.5] },
      ...
      // 4. 中+ボス「ウェイブロード」（~240秒）。波動砲（薙ぎビーム）＋アームスラム＋召喚＋phase2。
      {
        ...
        //   ぶつけてきたり」）。ウェイブロード＝海の王なので錨と鎖。他のどの攻撃とも違う点は
        //   **往復すること**＝撃たれた線が、避けた後にもう一度同じ線を戻ってくる。
        //   「1回避けれ ば終わり」ではなく「戻りも数える」＝1つの技で2回の判断が生まれる。
        ...
        // R52b: 108→165（+53%）。全ボスで最も遅い弾だった。
        //   ⚠️ +30%（140）でもまだ主人公(148)より遅い＝**壁から歩いて逃げ切れる**＝
        //      「抜け道を探して走る」という技の主旨そのものが成立しなかった。主人公より速い
        //      ことを下限にしてから幅を当てる。
        ...
        // R52b: 142→185（+30%）。射程据え置き（142×3.6＝511px → 185×2.8＝518px）。
      ...
      // 5. 大ボス「ミサイルガ」（~300秒）。ホーミングミサイル＋バルカン＋召喚＋phase2。
      {
        ...
        // R52b: 300→330（+10%）。既に速い側なので上げ幅は小さい。
        //   ⚠️ 旋回上限82°/秒は据え置き＝旋回半径は 300/1.431＝210px から 330/1.431＝231px へ
        //      **大きくなる**。「速くて怖いが、横へ切れば前より抜けやすい」（R31と同じ算定）。
        //      射程据え置き（300×3.5＝1050px → 330×3.2＝1056px）。
        missile: { telegraphSec: 0.6, count: 4, launchSpeed: 330, homingRate: 2.4, maxTurnDeg: 82,
                   speed: 330, radius: 6, damage: 24, blastDamage: 18, lifeSec: 3.2 },
        // ★R52b 署名攻撃②「ミニロボほうしゅつ」＝極小ロボの大量放出（ユーザー案「極小の小型ロボを
        //   大量に排出して、それらが襲ってくる」）。ミサイルガ＝運搬機なので「積んでいるものを出す」。
        //   ⚠️ ユーザー指定「**倒すだけ。ビリヤード弾にできない**」＝この子たちだけは
        //      よろけ(stagger)に入らず・掴めず・報酬も出ない。HP1なので武器でも突きでも一撃で消える。
        //      実装は Run.enterStagger／capture.onEnemyKilled／billiard.press の3か所で
        ...
        // R52b: 144→187（+30%）。射程据え置き（144×3.6＝518px → 187×2.8＝524px）。
        ring: { telegraphSec: 0.5, count: 11, count2: 14, bulletSpeed: 187,
                bulletRadius: 4, damage: 17, lifeSec: 2.8 },
        summon: { count: 7, enemyId: 'chibit', ringRadius: 68, telegraphSec: 0.6 },
        idleSec: { afterSpawn: 2.8, betweenAttacks: [2.2, 2.4, 2.8, 2.2, 2.2] },
      ...
      //    radius/glow/spawnDist も追随縮小（通常ボスの radius 64〜76 帯へ）。
      {
        ...
        // ★★R64 実プレイFB「マオウレクスは強い＝**削れない**」（息子さん）。実測して分かったのは
        //   手触りの話ではなく**計測環境の取り違え**だった：上の 3430 も真の姿の 3780 も
        //   「れんしゅうじょう＝主人公が無敵・雑魚がいない」で測ったDPSで、本番と同じ条件
        //   （雑魚あり・避ける・死ぬ）の実測は **396〜716/秒**（3シード×950秒）。同じHPが
        //   3〜6倍の尺になり、第1形態だけで実測 95〜172秒。真の姿に至っては 418〜495秒
        //   かけても45〜64%しか削れず＝**本番では誰もクリアできない**状態だった。
        //   → 68000 は「練習場で28秒」の値。本番実測DPS から引き直して 22000。
        //   実装後の実測（3シード・本番と同じ条件）：**32.7／34.5／52.4秒**で、その中に
        //   missile・wirearm・vulcan・knuckle・nova・分離・じゃがんレーザー・再合体・
        //   じゃしんレーザーが全部1回以上入る。しかも**3シード中2つが真の姿まで撃破**した
        //   （旧HPでは950秒＝本編の2.6倍走らせても0/3だった）。
        //   1発の重み（実測）：通常の投げ 1500〜1720＝**7〜8%**（68000では2.2%＝ゲージが
        //   動かない＝「削れない」の正体）／装甲片は上限5%×ブレイク2.4＝12%／らいこうだんは
        ...
        // ★R64 radius 27→38。実測で第1形態のDPSが 550→688（+25%）。理由は当たり判定の作りにある：
        //   玉は本体を**貫通**し、飛んだ線がコア円（半径 radius ＋ 玉の半径22.7〜28.4）を通れば当たる。
        //   27 だと通る帯が50〜55px しかなく、コアは swayX 30px で泳ぐので**帯とほぼ同じ幅だけ動く**＝
        //   狙っても運になっていた。38 なら帯61〜66px ＞ 泳ぎ30px＝「狙えば通る、外せば弾かれる」に戻る。
        //   実測（マオウ戦）：仲間の弾2098発・自動拳569発・素手114発は**全部カキン**で、通ったのは
        ...
        trueForm: {
          ...
          // ★★R64 240000 → 30000。上の「65.9〜84.3秒」は**れんしゅうじょう**（主人公が無敵・
          //   雑魚なし・ボスの前に立ちっぱなし）で測った数字だった。本番と同じ条件で測ると
          //   実測DPSは 186〜369/秒（練習場の3780の**1/10〜1/20**）で、418〜495秒かけても
          //   45〜64%しか削れない＝転生の見せ場に到達しても**終われない**。
          //   本番実測の中央値299/秒 × 目標100秒 ≒ 30000。30000での実測は **74.7／109.4／165.6秒**
          //   （中央値109秒）で、うち2つは撃破まで到達。
          // ★R64b ユーザー指示「40000にして」。30000＝中央値109秒に対し、40000は同じ実測DPS278で
          //   **約145秒**（＋殻の無敵10秒＝155秒）。最終ボスの尺を厚くする方向。旧240000（実測650〜1290秒
          //   ＝クリア不能）とは別物であることは変わらない。⚠️ 殻を割る閾値は**絶対値4500**を保つ必要が
          //   あるので、比率を 0.15 → 0.1125 へ同時に直してある（HPだけ動かすと割る遊びが消える）。
          //   真の姿は第1形態（22000）より
          ...
          // ★★R64 0.041 → 0.15。守るべきは比率でも旧い絶対値でもなく「**投げ1〜2発で割れる**」
          //   という関係そのもの。HP 240000×0.041＝9840 は「渾身のコア一投≒13000」を前提にした
          //   値だったが、本番実測の1発は 通常の投げ1500〜1720／装甲片3400（上限5%×ブレイク2.4）
          //   で、9840 は**5発ぶん＝窓1.4秒では絶対に届かない**＝殻を割る遊びは本番では
          //   一度も成立していなかった（実測：殻閉じ28〜30回に対し割れ1回）。
          //   40000×0.1125＝**4500**＝装甲片1枚＋投げ1発、または投げ3発。窓1.4秒に届く側へ置く。
          //   ⚠️ HPを動かすときは必ずこの**絶対値4500**が保たれるよう比率も直すこと（R64b で 0.15→0.1125）。
          shell: { teleSec: 0.7, closeSec: 1.4, holdSec: 1.7, openSec: 0.6, breakSec: 2.0,
                   interruptRatio: 0.1125,
  ...
  // ★1めんボスおためし（タイトルの R キー）。実プレイFB「画面内に情報量が多くて処理しきれない」の
  //   A/Bテスト用。⚠️ユーザー指示（2026-09-02）「短縮版にしないで。通常版の一面
  //   （ゲーム開始〜コロガンナー撃破）を通して比べたい」＝出現時刻・装備・中身はすべて本番と同じ。
  //   おためし固有なのは「Iキーの情報レベル」と「撃破で終わってタイトルへ戻る」だけ。
  ...
  rareEnemy: {
    ...
    // ★R46 実プレイFB「ボス戦中だけ間隔を詰めて。ボス戦でこそ真価を発揮する」。
    //   実測（R45・自然プレイ315秒）：出た5体のうち**ボス戦中に出たのは1体だけ**だった。
    //   ⚠️ 平常時の間隔（38〜70秒）は触らない。2026-08-23に「マグマンが少し多い」と
    //      指摘されて34→54秒へ伸ばした経緯があるので、総量を戻すのは逆行になる。
    //      ★増やすのではなく**寄せる**：ボス戦のあいだだけ間隔を約半分にする。
    bossFirstSec: 6,              // ボス戦に入った瞬間、待ちをここまで切り詰める
                                  //（長い待ちの途中でボスが始まっても、その戦闘に間に合う）
    bossEveryMin: 20, bossEveryMax: 34,   // ボス戦中の間隔（平常の約半分）
...
BALANCE.boss.jamTiers = [
  {
    ...
    // ★2026-09-13 実プレイFB「破片がどこにあるかまったくわからない」（37回目・装甲片を返した 0）。
    //   正体は聖歌隊（ringRadius 70）と同じ：本編共通の hero.billiard.shards.dist 62 は大聖堂の当たり 88・絵の幅 300px の
    //   **内側**＝巨体の絵の下に湧いて隠れていた。拾いに行けば接触ダメージ（巨体が3回連続で被ダメ上位の一因）。
    //   → 大聖堂だけ主人公との間・体の外（200px＝絵の半幅 150 の外。175 だと腕の絵に重なった＝jam-shard-shots で確認）へ
    //     飛び出させ、掴める時間も 4.5→9 秒（billiard.dropShards が読む）。
    shardDist: 200, shardHoldSec: 9,
    glowOuter: '#c9971f', glowInner: '#1f47b8',
    // ★2026-09-14 実プレイ41・42回目＝巨体（接触）が 95(5)・140(10)＝1発14〜19で死因2位、過去6回 76〜192 で下がらない。
    //   装甲片・欠片・聖歌隊は拾いに行く設計（近づく必要がある）のに大聖堂は chaseSpeed 40 で歩いて寄る＝拾う・溜める間に
    //   体が重なる。避ける技術でなく設計の接近要求に払っている分なので 24→16（本編ボスの 12〜30 の中位）。ユーザー承認済み。
    chaseSpeed: 40, bodyDamage: 16,
    ...
    pillar: { telegraphSec: 0.7, count: 3, interval: 0.6, radius: 44, damage: 26, leadSec: 0.3, height: 250,
              ...
              // ★2026-09-14 FB「光の柱をもっと太く、色をもっと濃く」→ 白に近かった淡色を飽和した色へ（白金→黄金・橙→朱・深紅→血赤）。
              tints: ['#ffc21a', '#ff6a00', '#e0001e'], edgeTints: ['#a86a00', '#c83c00', '#7a0012'],
