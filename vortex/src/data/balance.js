...
export const BALANCE = {
  ...
  //   A/Bテスト用に、コロガンナー戦だけを短時間で何度も試せるようにする。
  //   ⚠️ここは**おためしモード専用**の値。通常プレイ・れんしゅうじょう・autotest からは1つも読まれない
  //     （読む側は run.trialMode でゲートしてある）＝本番のバランスは1ドットも動かない。
  trial: {
    warnSec: 8, spawnSec: 10,   // 開幕10秒でコロガンナー（本番は58/60秒）＝待たずに何度も比べられる
    weaponLevel: 3,             // 開始装備。素の Lv1 だと倒しきる前に飽きるので少しだけ上げる
