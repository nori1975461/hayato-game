...
export const BALANCE = {
  ...
  hero: {
    ...
    billiard: {
      ...
      // 湧きの補正。⚠️ 一度 0.55 まで削ったが、実プレイFBを読み直して 0.85 へ戻した。
      //   「敵が多すぎる」（第一印象）→「慣れると、はるかに戦っている緊張感がある」（数回後）
      //   →「ビリヤード攻撃が敵の数と比較して弱すぎる」
      //   つまり問題は敵の数ではなく攻撃の弱さ。緊張感は②の合格基準そのものなので削ってはいけない。
      //   実測でも 0.55 では場の敵が11体しかおらず、炸裂の連鎖相手が居ないので投げが逆に弱くなった。
      // 0.85 → 0.70（実プレイFB「敵をもう少し減らして」）。
      // ⚠️ 一度 0.55 まで削ったときは場の敵が11体しか居らず、炸裂の連鎖相手が消えて
      //    投げが逆に弱くなった（平均2.93体）。減らしすぎは①を殺すので中間の0.70で止める。
