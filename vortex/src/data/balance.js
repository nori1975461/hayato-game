      ...
      //    radius/glow/spawnDist も追随縮小（通常ボスの radius 64〜76 帯へ）。
      {
        ...
        trueForm: {
          ...
          rage: {
            ...
            shadowAdd:   [0, 1, 1, 2],              // R44W5 かげおに 2→3→3→4体（旧 wavesAdd の後継）
          ...
          //     主人公の**足あと（通った道）**から**倒立した主人公自身の影**が起き上がり、
          //     主人公の過去の動きをそのまま speedMul 倍速で再生しながら追ってくる。
          //   悪魔性＝**自分自身が敵になる**（幻影・憑依の古典）。語彙は堕ちた聖句と同じ
          //   **倒立**と**紫→深紅**。名前は影踏み鬼＝ルールが名前で伝わる（とまるな！）。
          //   やや理不尽の正体＝**足を止めると必ず捕まる**：影は自分の過去位置の再生なので、
          //     止まれば minGapSec 後に自分の現在地へ届く。ただし床（minGapSec）があるので
          //     動いている限り追いつかれない＝かかとを噛まれ続ける緊張だけが残る。
          //   ★minGapSec 0.55 の根拠＝**主動詞を否定しない**。溜め中は 0.5倍（74px/s）・投球
          //     モーション0.22秒は 0.25倍が既存の「②のアンカー」としてあるので、
          //     床0.35だと**投げるたびに噛まれる**（実測で判明）。0.55なら
          //     走行81px・溜め歩き41px ＞ 接触18px＝投げても溜めても逃げ切れる。
          //     捕まるのは**完全に立ち止まった時だけ**（0.55秒×停止≒18px内）＝ルールが1つ。
          //   gapStepSec＝影ごとに床をずらす。無いと全員が同じ床に達して**1点に重なり1体に
          //     見える**（実測で判明）。0.22ずつずらすと約33px間隔の「堕ちた自分の隊列」になる。
          ...
          shell: { teleSec: 0.7, closeSec: 1.4, holdSec: 1.7, openSec: 0.6, breakSec: 2.0,
                   ...
                   shadow: { count: 2, delaySec: [1.2, 2.0, 2.8, 3.6], speedMul: 1.30,
                             minGapSec: 0.55, gapStepSec: 0.22, lifeSec: 5.5, damage: 26,
                             radius: 11, riseSec: 0.55, novaRadius: 64, novaDamage: 16,
                             // 炸裂の前に flareSec だけ**その場に静止**して膨らむ。静止しないと
                             // 走っている人の 81px 後ろで爆ぜて回避不能（爆風71px・実測で判明）。
                             // 静止すれば走行 0.6×148=89px ＞ 爆風71px＝正しく走っていれば必ず躱せる。
                             flareSec: 0.6 } },
        },
