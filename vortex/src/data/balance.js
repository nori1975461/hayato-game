  ...
  boss: {
    ...
    tiers: [
      ...
      //    radius/glow/spawnDist も追随縮小（通常ボスの radius 64〜76 帯へ）。
      {
        ...
        // ⚠️ 尺の設計＝ユーザー指定「20〜25秒（短く強烈）」。R34 の実測（HP120000 で 37.2秒）から
        //    プレイヤーの実DPSは約3226/秒。殻閉じの無敵（1.4秒×約2回＝2.8秒）を引いた実ダメージ時間
        //    19.7秒 ぶんで 63500。眼が中央固定（sway 0）で狙いやすくなるぶん +15% を見て 73000 …
        //    ではなく、初期値は 68000 から入れて**実測で詰めた**（見積もりのまま置かない）。
        //    実測①68000＝21.3秒／②68000＝17.5秒（どちらも殻閉じが1回も成立せず無敵が計上されていない）
        //    ／③62000＝18.5秒（殻閉じ成立あり）。
        //    ⚠️ 同じ設定でも ±10% ぶれる（最後の1手に入れるかどうかで ±2秒動く）ので、秒数ではなく
        //      **実DPSの平均 3550/秒** から引き直す。目標22.5秒－無敵1.4秒＝21.1秒ぶん → 75000。
        // ⚠️ 攻撃は3種だけに絞る。20秒台の戦いに5種を積むと1種あたり1回も回ってこない
        //    ＝R34で踏んだ「再生される前に終わる」と同じ失敗になる。3種なら各2回以上出る。
        trueForm: {
          ...
          hp: 75000, radius: 92, spriteScale: 7.4, glowScale: 10.6,
          glowOuter: '#c98cff', glowInner: '#ffedb0',
          chaseSpeed: 110, bodyDamage: 38, gaugeSegments: 2,
          bulletTint: '#ffedb0',
