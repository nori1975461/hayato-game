...
export function createBilliard(run) {
  ...
  function doThrow(ang) {
    ...
    const hp = h.maxHp + Math.round(b.chargeHpBonus * ratio);
    ...
    run.shake(60 + 70 * ratio, 2 + 3 * ratio);
  }
  ...
  function hitOne(s, e) {
    if (e.stag) {
      ...
      const r = run.burstStagger(e.x, e.y);
      s.kills += r.total;
  ...
  function burstEnd(s) {
    ...
    if (s.kills > 0) {
      const P = run.hitFeel();
      // 振幅は頻度と逆相関。投げは0.4〜0.8回/秒＝一撃の3〜7分の1なので、大きく出してよい。
      const sh = s.kills >= 3 ? P.chainShake : P.breakShake;
      if (sh && sh[0] > 0) run.shake(sh[1], sh[0]);
      if (!run.cinematic) {
        run.freezeT = Math.min(BALANCE.hitFeel.freezeCapSec,
          Math.max(run.freezeT, s.kills >= 3 ? 0.16 : 0.075));
      }
  ...
  function toggleMode() {
    ...
    if (st.mode === 0) { st.held = null; st.chargeT = 0; hideHeld(); run._moveMul = 1; }
