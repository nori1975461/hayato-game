...
export function createCapture(run) {
  ...
  // ★R47 SHIELD/SPEED/SLEEPY を追加。R45 でこの3体を足したときに入れ忘れていた漏れで、
  //   命の盾（マモリン）と爆速ドリンク（ドリンゴ）が**合成で勝手に別の攻撃役へ化けて消える**
  //   状態だった＝ビリッコで実測したのと同じ事故が、そのまま新3体で再現する構造。
  //   プレイヤーが選んだ役割は合成で奪わない（このリストの趣旨そのもの）。
  const NON_COMBAT = ['HEAL', 'AMMO', 'SHIELD', 'SPEED', 'SLEEPY'];
