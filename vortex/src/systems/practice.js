...
// R35: テンポ違いの3つ（184/208/168）は3つとも「違う」と言われた＝テンポは軸ではなかった。
// 今回は作曲を共通にしたまま**編成そのものを変えた3つ**にして、どれが選ばれるかで
// 「音色が原因だったのか」を確かめる。
// R36W2 実プレイFB「BGMのVersionはギターでいこう」＝①が本採用（本編の既定はもともと①）。
// 一覧は消さずに残し、④に軌道神核の専用曲を足した＝転生まで進まなくても B キーで聞ける。
// R52W2: 作り直した**共通ボス曲**（マオウレクスより前の5体）を2案ここへ足す。本編では60秒
//   待たないと鳴らないので、聴くだけのために1面を通す必要があった＝聞き比べの場所が無かった。
//   ⑤＝本編で鳴る案（オーケストラルロック／参考＝ロマンシングサガ Re;univerSe）、
//   ⑥＝対比用の電子系（ダークシンセ）。どちらが良いかを B キーで選んでもらう。
const MAOU_BGM = [
  { name: 'maou',      label: '① ギター（ひずみ）★さいよう' },
  { name: 'maouOrch',  label: '② オーケストラ' },
  { name: 'maouSynth', label: '③ シンセ（しっそう）' },
